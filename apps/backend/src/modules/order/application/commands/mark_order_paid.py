"""Consumer-driven command: PENDING → PAID after capture event."""
...
class MarkOrderPaidHandler:
    def __init__(
        ...
        self._uow = uow
        self._logger = logger.bind(handler="MarkOrderPaidHandler")

    async def handle(self, command: MarkOrderPaidCommand) -> None:
        async with self._uow:
            ...
            if order.status != OrderStatus.PENDING:
                self._logger.info(
                    "order.mark_paid.noop",
                    order_id=str(order.id),
                    status=order.status.value,
                )
                return
            pre = order.status
            order.mark_paid(payment_intent_id=command.payment_intent_id)
            ...
            self._logger.info("order.paid", order_id=str(order.id))
