...
be automated. The user must confirm the action directly.
    ...
    parser.add_argument(
        "--service", required=True, help="Service name (requires linked project)"
    )
    ...
    if library_already_loaded:
        ...
        info(
            "pg_stat_statements is NOT in shared_preload_libraries (will need restart)"
        )
        needs_restart = True
    ...
    if needs_install:
        ...
        code, output = run_psql_query(
            service, "CREATE EXTENSION IF NOT EXISTS pg_stat_statements"
        )
        if code != 0:
    ...
    if needs_restart:
        ...
        code, stdout, stderr = run_railway_command(  # ty: ignore[unresolved-reference]  # noqa: F821
            [
                "restart",
                "--service",
                service,
