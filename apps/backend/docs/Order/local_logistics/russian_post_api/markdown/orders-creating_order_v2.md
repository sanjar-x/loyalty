...
| Параметр | Значение |
|----------|----------|
| Локальный URL | **/2.0/user/backlog** |
| Тип | **PUT** |
...
Тело запроса идентично версии 1 (см. [Создание заказа](orders-creating_order.md)).
...
| tender | Логические: true или false (Опционально) | Доступно только для отправлений «ЕКОМ Маркетплейс» |
...
| orders | Массив (Опционально) | Созданные заказы |
| orders[].barcode | Строка | ШПИ отправления |
| orders[].order-num | Строка | Номер заказа. Внешний идентификатор заказа, который формируется отправителем |
| orders[].result-id | Целое число | Внутренний идентификатор заказа |
| orders[].group-name | Строка (Опционально) | Идентификатор группы |
