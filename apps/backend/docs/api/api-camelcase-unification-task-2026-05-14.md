...
Всего **55 схем** на голом `BaseModel`, ни одной на `CamelModel`: 8 в `schemas_admin.py` (provider-accounts), 47 в `schemas.py` (shipments / rates / pickup-points / intakes / edit-tasks / returns).
...
Рекомендация: вынести в **отдельную под-фазу** (см. §4, фаза 3). Менять их независимо breaking, FastAPI требует `Query(alias=...)`, и для path-параметров (`{product_id}`) выгода чисто косметическая — **path-параметры не трогать**. Прецедент алиасинга query-параметра в кодовой базе уже есть — `router_admin_shipments.py:408` (`Query(..., alias="status")`), так что Фаза 3 опирается на существующий паттерн, а не вводит новый.
...
- **Не потерять per-class `model_config`.** Часть схем уже несёт свой `ConfigDict` — например `ProviderAccountResponse` (`schemas_admin.py:34`) имеет `from_attributes=True`. При переводе на `CamelModel` его нужно либо смержить per-class (`ConfigDict(from_attributes=True, alias_generator=to_camel, populate_by_name=True)`), либо добавить `from_attributes=True` в сам `CamelModel`. Простой `class X(CamelModel)` без проверки молча потеряет существующий конфиг.
...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `frontend-admin`                       | `shared/api/geo/*` (geo read-models); `features/order-actions/ui/ChangePickupPointModal.jsx` + `app/api/admin/logistics/pickup-points` (`PickupPointSchema`); BFF-прокси отдают JSON «как есть» — поломаются все entity/feature-слои, читающие snake_case-поля логистики/гео | Обновить чтение полей; пересобрать `openapi/openapi.json`                                                        |
