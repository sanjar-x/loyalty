...
| `full_address`     | string                                                 | Нет          | Полный адрес с указанием города, улицы и номера дома.     | —                     | —                                                                             | `Санкт-Петербург, Большая Монетная улица, 1к1А` |
| `self_pickup_id`   | string                                                 | Нет          | ID ПВЗ в логплатформе                                     | —                     | —                                                                             | `01946f4f013c7337874ec2fb848a58a4`              |
| `last_mile_policy` | [OffersInfoLastMilePolicy](#offersинfolaлstmilepolicy) | Нет          | Требуемый способ доставки                                 | `time_interval`       | `time_interval` —интервал доставка до двери; `self_pickup` — до пункта выдачи | —                                               |
| `is_oversized`     | boolean                                                | Нет          | Флаг КГТ (крупногабаритный товар)                         | —                     | —                                                                             | —                                               |
| `send_unix`        | boolean                                                | Нет          | Формат интервалов доставки (`true` — unix, `false` — utc) | —                     | —                                                                             | —                                               |
