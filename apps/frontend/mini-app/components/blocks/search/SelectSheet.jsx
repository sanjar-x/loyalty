...
export default function SelectSheet({
  ...
  const resolvedControl = control === 'auto' ? (multiple ? 'check' : 'radio') : control;
...
function SelectSheetStateful({
  ...
  const displayOptions = useMemo(() => {
    ...
    const filteredFlat = q ? flat.filter((opt) => normalizeText(opt.label).includes(q)) : flat;
    ...
    for (const opt of rawOptions) {
      ...
      if (!normalizeText(opt.label).includes(q)) continue;
    ...
    >
      ...
      <div className={styles.list} role={multiple ? 'group' : 'radiogroup'}>
        {(displayOptions || []).map((opt, idx) => {
          ...
          const controlClass = selected ? styles.checkSelected : styles.checkUnselected;
