...
import { useGetProductByIdQuery, useAddCartItemMutation } from '@/lib/store/api';
import { normalizeApiError, humanizeApiError } from '@/lib/api/errors';

import styles from './QuickAddSheet.module.css';

// Cart sahifasi — `/cart` route. `Купить сейчас` shu yerga
...
export default function QuickAddSheet({ product, productSlug, open, onClose }) {
  ...
  // Sheet `ProductCard` tomonidan `isQuickAddOpen ? <Sheet/> : null` pattern bilan
  ...
  async function handleBuyNow() {
    const ok = await performAdd();
    if (!ok) return;
    // "Купить сейчас" — savatga o'tkazadi (`/cart`), sheet darhol yopiladi.
    onClose?.();
