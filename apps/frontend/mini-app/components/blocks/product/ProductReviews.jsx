...
export default function ProductReviews({
    ...
    return (
      <div className={styles.stars} aria-label={`Рейтинг ${safeRating} из 5`}>
        {Array.from({ length: starsToRender }, (_, i) => i + 1).map((star) => (
          <Image key={star} src="/icons/product/Star.svg" alt="star" width={12} height={12} />
        ))}
      </div>
    );
  };
  ...
  const reviewsHref = brandSlug
    ...
    : '/profile/reviews';
  const previewImages = Array.isArray(productImages) ? productImages.slice(0, 2) : [];
        ...
        <div className={styles.summary}>
          <div className={styles.ratingBlock}>
            <div className={styles.ratingRow}>
              ...
              <Image src="/icons/product/Star.svg" alt="star" width={14} height={14} />
            </div>
          ...
          {previewImages.length > 0 ? (
            <div className={styles.previewImages} aria-hidden="true">
              {previewImages.map((src, idx) => (
                <span
                  key={`${src}-${idx}`}
                  className={cx(styles.previewImgWrap, idx === 1 && styles.previewImgSecond)}
                >
                  <Image src={src} alt="" fill className={styles.previewImg} sizes="56px" />
                </span>
              ))}
            </div>
          ) : null}
                  ...
                  <article key={review.id} className={styles.reviewCard}>
                    ...
                    <div className={styles.reviewBody}>
                      <p className={styles.reviewText}>
                        <span className={styles.reviewLabel}>Достоинства:</span> {review.pros}
                      </p>
                      <p className={styles.reviewText}>
                        <span className={styles.reviewLabel}>Недостатки:</span> {review.cons}
                      </p>
                    </div>
                  </article>
                ))
