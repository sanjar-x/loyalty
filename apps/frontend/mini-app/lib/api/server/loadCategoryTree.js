...
import { mapCategoryTree } from '@/lib/adapters/mapCategoryTree';
...
export const loadCategoryTree = cache(async (opts = {}) => {
  ...
  const url = `${base}/api/v1/storefront/categories/tree${qs ? `?${qs}` : ''}`;
  ...
  if (!res.ok) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      console.error(`[loadCategoryTree] upstream ${res.status}`, body.slice(0, 300));
    }
    return [];
  }
