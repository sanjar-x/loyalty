...
/**
 * Bosh sahifa filtr domeni — audit #1: `app/page.jsx` god-komponentidan
 * ajratildi (~160 satr). Egallaydi:
 *  • filtr qiymat state'i (category/sort/type/brand/price/delivery/original)
 *  • metadata so'rovlari (categories darhol; types/brands lazy, skip-gated)
 *  • barcha derive: options, chip-label'lar, count, id→name map'lar
 *  • `hasActiveFilters` / `showResults` — sahifa mode hisoblovi va skip-gate
 *
 * `commitSearch` / `handleCategoryChange` / `resetToHome` — qidiruv state'iga
 * ham tegadi, shu sabab sahifada glue sifatida qoladi. Bu hook ularga
 * `setActiveCategoryId` va h.k. + `categoriesRef`/`brandsRef`'ni beradi
 * (oxirgi ro'yxat — dep-churn'siz exact-match lookup uchun).
 *
...
export function useHomeFilters({ submittedQuery, isFiltersOpen, isTypeOpen, isBrandOpen }) {
  // `commitSearch` (sahifa glue) shu ref'lardan o'qiydi — `brandsList`/
  // `categories`'ni dep array'ga qo'shmasdan, oxirgi qiymatni oladi.
   ...
   * Home mode'da bu ro'yxatlar kerak emas — product card'lar brand/type
   * ma'lumotini backend javobi ichida olib keladi. Cache 30 daqiqa
  ...
  // Latest-value ref mirror — render paytida emas, effekt orqali (react-hooks/refs).
  // `commitSearch` (sahifa glue) commit'dan keyin, user event'da o'qiydi.
  useEffect(() => {
  ...
  // Latest-value ref mirror — effekt orqali (yuqoridagi `categoriesRef` izohi).
  useEffect(() => {
