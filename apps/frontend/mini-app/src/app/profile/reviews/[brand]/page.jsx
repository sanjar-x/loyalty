...
function Segmented({ value, onChange, items }) {
  return (
    <div className={styles.segmented} role="tablist" aria-label="Фильтр отзывов">
      {items.map((it) => {
      ...
      {review.product ? (
        <div className={styles.productRow}>
          <div className={styles.productThumb} aria-hidden="true">
            <img src={review.product.image} alt="" className={styles.productThumbImg} />
          </div>
          <div className={styles.productMeta}>
            <div className={styles.productName}>{review.product.name}</div>
            <div className={styles.productSub}>
              Размер: <span>{review.product.size ? `${review.product.size}` : null}</span> ·{' '}
              {review.product.size && review.product.article ? '' : null}
              Артикул: <span>{review.product.article ? `${review.product.article}` : null}</span>
            </div>
          </div>
        </div>
      ) : null}
...
export default function BrandReviewsPage() {
  ...
  // Backend Reviews module doesn't exist yet (Spec §11). When the module is
  // ready, this will be wired to RTKQ `useBrandReviewsQuery({ brand })` and
  // the shape will be adapted here.
  const { priceRub, averageRating, totalReviews, ratingBars, reviews } = useMemo(
    () => ({
      priceRub: 0,
      averageRating: 0,
      totalReviews: 0,
  ...
  const filtered = useMemo(() => {
    ...
    if (onlyPhoto) list = list.filter((r) => Array.isArray(r.images) && r.images.length > 0);
        ...
        <main className={styles.main}>
          <div className={styles.summaryCard}>
            <div className={styles.summaryTop}>
              ...
              <div className={styles.ratingBars} aria-label="Распределение рейтингов">
                {ratingBars.map((b) => (
              ...
              {sortOptions.map((opt) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={
                        selected ? styles.sheetControlSelected : styles.sheetControlUnselected
                      }
                      aria-hidden="true"
                    >
                      {selected ? (
          ...
          <div className={styles.list}>
            {filtered.length ? (
              ...
              <div className={styles.emptyCard} role="status" aria-live="polite">
                ...
                <button type="button" className={styles.emptyHint} onClick={() => setTab('all')}>
          ...
          <div className={styles.brandHint}>{brand ? `Отзывы бренда ${brand}` : 'Отзывы'}</div>
        </main>
