...
export default function PromoCodesPage() {
  ...
  // Backend Promo module doesn't exist yet (Spec §11) — empty list. When the
  // module is ready, populated via `usePromoCodesQuery` from RTKQ. Empty state
  // is already in place — it will be filled automatically once promo codes arrive.
  ...
  const doCopy = useCallback(async () => {
    ...
    if (tg?.showPopup) {
      try {
        tg.showPopup({
          message: 'Не удалось скопировать промокод. Попробуйте выделить и скопировать вручную.',
        });
      } catch {
        // ignore
      }
      ...
      >
        <div className={styles.sheetContent}>
          <div className={styles.sheetTop}>
            <div className={styles.sheetTitle}>Промокод {activePromo?.code}</div>
            <button
              type="button"
