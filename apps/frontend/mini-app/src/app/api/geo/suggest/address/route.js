...
/**
 * Address suggest BFF — now a proxy to our backend's custom geo catalog.
 *
 * Old behavior: request to the `dadata.ru` Suggestions API.
 * New behavior: `GET /api/v1/geo/countries/RU/subdivisions?search=...&lang=ru`
 * — searches by district/region name translated by the backend and
 * returns the result together with lat/lng.
 *
 * The response shape is kept compatible with DaData, so the
 * `app/checkout/pickup/page.jsx` client works unchanged:
 *
 *   {
 *     suggestions: [
 ...
 *   }
 *
 * Limitation: the backend has no single-call district-level search across
 * the whole country (only per-subdivision). For the current UX (large
 * cities/regions) the subdivision level is enough (Moscow, Saint
 * Petersburg, Tatarstan, etc. — Russian Federation subjects). If granular
 * district search is needed, you can drill into districts after a
 * subdivision is selected in the future.
 */
