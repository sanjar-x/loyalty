...
// Referrals — backend not yet shipped (Spec §11). Inline stubs: empty data
// + graceful UI render. When the backend is ready — replace with real
// RTKQ hooks from entities/referral (the slice was removed in Sprint 1.1).
...
const useGetMyReferralLinkQuery = () => ({ ...pendingFeatureQuery(), data: null });
const useGetMyInvitedUsersQuery = () => ({ ...pendingFeatureQuery(), data: [] });
const useGetMyActiveDiscountQuery = () => ({ ...pendingFeatureQuery(), data: null });
const useGetMyReferralStatsQuery = () => ({
  ...pendingFeatureQuery(),
...
export default function InviteFriends() {
  ...
  const invitedUsers = Array.isArray(invitedData?.items) ? invitedData.items : [];
  ...
  const jsonLd = {
    ...
    mainEntity: {
      ...
      description: 'Пригласите 3 друзей в приложение и получите промокод на скидку 10%',
      eligibleQuantity: {
        ...
        <section className={styles.hero} aria-label="Invite friends">
          <div className={styles.illustration} aria-hidden="true">
            <div className={styles.bubbles}>
              <Image
                src="/icons/invite-friends/1-avatar.webp"
                alt=""
                width={51}
                height={51}
              ...
              />
              <Image
                src="/icons/invite-friends/2-avatar.webp"
                alt=""
                width={50}
                height={50}
              ...
              />
              <Image
                src="/icons/invite-friends/3-avatar.webp"
                alt=""
                width={54}
                height={53}
              ...
              <Image
                src="/icons/invite-friends/4-avatar.webp"
                alt=""
                width={50}
                height={50}
              ...
              />
              <Image
                src="/icons/invite-friends/5-avatar.webp"
                alt=""
                width={55}
                height={55}
              ...
              />
              <Image
                src="/icons/invite-friends/6-avatar.webp"
                alt=""
                width={50}
                height={50}
          ...
          <p className={styles.subtitle} itemProp="description">
            Пригласите в приложение 3 друзей по своей ссылке и мы подарим вам промокод на скидку
            10%.
          </p>
        </section>

        {isDiscountPending ? (
          <section className={styles.promoCard} aria-label="Промокод" aria-busy="true">
          ...
          <ul className={styles.historyList}>
            {isInvitedPending
              ? Array.from({ length: 3 }).map((_, idx) => (
                  <li key={`sk-${idx}`} className={styles.historyItem} aria-hidden="true">
              ...
              : invitedUsers.map((user) => {
                  ...
                  const name =
                    ...
                    user?.invitee_tg_id ||
                    (user?.invitee_id != null ? String(user.invitee_id) : 'Пользователь');
                  const date = formatRuDateTime(user?.created_at);

                  return (
                    <li
                      key={user?.invite_id ?? user?.invitee_id ?? user?.invitee_tg_id ?? name}
