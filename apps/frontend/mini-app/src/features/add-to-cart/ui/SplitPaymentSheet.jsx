...
export default function SplitPaymentSheet({ open, onClose, price, splitPayment }) {
  const total = toNumber(price);
  const fallbackAmount = toNumber(splitPayment?.amount);

  const terms = useMemo(() => {
    // If the backend later returns terms/plans, this logic can easily be swapped out here.
    return [
      {
        id: '2m',
        title: '2 месяца',
  ...
  const header = (
    <div className={styles.header}>
      ...
      <div className={styles.headerRow}>
        <button type="button" className={styles.backBtn} aria-label="Назад" onClick={onClose}>
      ...
      <div className={styles.body}>
        <div className={styles.group} role="radiogroup" aria-label="Срок">
          {terms.map((t) => {
            ...
            const intervalDays = Math.max(7, Math.round((months * 30) / Math.max(1, count - 1)));
            const progressPercent = Math.min(100, Math.max(0, (1 / count) * 100));
                      ...
                      {Array.from({ length: count }).map((_, idx) => (
                        <div key={idx} className={styles.scheduleItem}>
                          <div className={styles.scheduleDate}>
                            {idx === 0
                              ? 'Сегодня'
                              : formatRuShortDate(addDays(new Date(), idx * intervalDays))}
                          </div>
                    ...
                    <div className={styles.progress} aria-hidden="true">
                      ...
                      {Array.from({ length: howMany }).map((_, index) => (
                        <span
                          key={index}
                          className={
                            index < filledCount ? styles.progressFill : styles.progressNotFill
                          }
                          style={{ width: progressWidth }}
                        />
                      ))}
        ...
        <div className={styles.hint}>
          Выбрано: <span className={styles.hintStrong}>{selectedTerm.title}</span>
        </div>
      </div>
    </BottomSheet>
  );
