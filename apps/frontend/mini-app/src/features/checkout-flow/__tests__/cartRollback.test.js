/**
 * CHK-004: cartRollback helper unit tests.
 *
 * `useCheckoutFlow.placeOrder` scenarios:
 *   1) 5 items, 3 selected, initiate fail → restore 2
 *   2) 5/3, confirm fail → restore 2
 *   3) 5/3, success → no restore (so that the snapshot doesn't arrive empty
 *      to the helper, placeOrder calls `clearRemovedSnapshot()` after
 *      success — the helper may never be called)
 *   4) 5/3, 1 add fails → the remaining 1 is restored (best-effort)
 *   5) 5/5 → prepareCart early return → snapshot empty → helper no-op
 *
 * At the helper level we reproduce scenarios 1-5 with different snapshot and
 * mutation mocks.
 */
...
describe('restoreCartItems (CHK-004)', () => {
  ...
  it('scenario 3: success — placeOrder does not call helper (snapshot stays cleared)', async () => {
    // The helper itself: empty snapshot → 0 calls.
    const trigger = vi.fn();
