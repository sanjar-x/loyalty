...
import type { StoryShareParams, DownloadFileParams, EventType } from '../types';

export function useShare() {
  const isAvailable = supportsFeature('shareToStory') || supportsFeature('shareMessage');

  const shareToStory = useCallback((mediaUrl: string, params?: StoryShareParams) => {
  ...
  const shareMessage = useCallback((msgId: string): Promise<boolean> => {
  ...
  const downloadFile = useCallback((params: DownloadFileParams): Promise<boolean> => {
