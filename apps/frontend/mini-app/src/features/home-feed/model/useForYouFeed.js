...
import { useGetForYouFeedQuery, useLazyGetForYouFeedQuery } from '@/entities/product';
import { mapProductCard } from '@/entities/product';

/**
 * Home page "For you" personalized feed — cursor pagination,
 * IntersectionObserver infinite-scroll, exponential-backoff auto-retry on
 * transient errors. Audit #1: extracted from the `app/page.jsx`
 * god-component (~150 lines).
 *
 * `sentinelRef` is returned — the page attaches it to the `<div>` at the
 * end of the feed. The IntersectionObserver activates only in `isHomeMode`.
 *
...
export function useForYouFeed({ isHomeMode }) {
  ...
  useEffect(() => {
    ...
    if (!initialProductsItems.length) return;
    if (!recommendedHasMore) return;
    // Don't re-trigger while an error is surfaced — user must click "Retry"
