...
function priceChipLabel(range) {
  ...
  if (min != null && max != null) return `От ${formatNumber(min)} ₽ до ${formatNumber(max)} ₽`;
...
export default function FiltersSheet({
  ...
  useEffect(() => {
    ...
    if (open && !wasOpen) {
      frame = requestAnimationFrame(() => {
        setDraft(initial);
        setMinDigits(initial.priceRange?.min ? String(initial.priceRange.min) : '');
        setMaxDigits(initial.priceRange?.max ? String(initial.priceRange.max) : '');
      });
    }
  ...
  const changed = useMemo(() => {
    ...
    const sameCategory = aCats.size === bCats.size && Array.from(aCats).every((x) => bCats.has(x));
    ...
    const sameTypes = aTypes.size === bTypes.size && Array.from(aTypes).every((x) => bTypes.has(x));

    const aBrands = new Set(a.brands || []);
    const bBrands = new Set(b.brands || []);
    const sameBrands =
      aBrands.size === bBrands.size && Array.from(aBrands).every((x) => bBrands.has(x));

    return !(sameCategory && sameTypes && sameBrands && samePrice && sameDelivery && sameOriginal);
  ...
  const minLabel = useMemo(() => {
    const p = priceBounds?.min != null ? `От ${formatNumber(priceBounds.min)} ₽` : 'От';
    return p;
  }, [priceBounds?.min]);

  const maxLabel = useMemo(() => {
    const p = priceBounds?.max != null ? `До ${formatNumber(priceBounds.max)} ₽` : 'До';
    return p;
  }, [priceBounds?.max]);
  ...
  const [isOriginalInfoOpen, setIsOriginalInfoOpen] = useState(false);
  ...
  return (
    <>
      <BottomSheet isFilter={true} open={open} onClose={onClose} title={title}>
        <div className={styles.body}>
          {selectedChips.length ? (
            <div className={styles.selectedChips} aria-label="Выбранные фильтры">
          ...
          <div className={styles.section}>
            ...
            <div className={styles.segmented} role="group" aria-label="Категория">
              {categories.map((c) => {
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDraft((prev) => {
                        ...
                        return {
                          ...prev,
                          categoryIds: active ? ids.filter((id) => id !== c.id) : [...ids, c.id],
                        };
                      })
                    }
                  >
          ...
          <div className={styles.section}>
            ...
            >
              <div className={styles.fieldValue}>{joinOrPlaceholder(draft.types, 'Не выбран')}</div>
              {draft.types?.length ? (
                <span
          ...
          <div className={styles.section}>
            ...
            <div className={styles.chipRow}>
              <button
                type="button"
                className={draft.delivery?.inStock ? styles.toggleChipActive : styles.toggleChip}
                onClick={() =>
                  setDraft((prev) => ({
                    ...prev,
                    delivery: {
              ...
              <button
                type="button"
                className={draft.delivery?.fromChina ? styles.toggleChipActive : styles.toggleChip}
                onClick={() =>
                  setDraft((prev) => ({
                    ...prev,
                    delivery: {
            ...
            <div className={styles.switchRow}>
              <div className={styles.switchLeft}>
                ...
                <button
                  ...
                  onClick={() => setIsOriginalInfoOpen(true)}
      ...
      <BottomSheet
        open={isOriginalInfoOpen}
        onClose={() => setIsOriginalInfoOpen(false)}
        title="Оригинал"
        footer={
          <div className={styles.footerRow}>
            <button
            ...
            <button
              ...
              onClick={() => setIsOriginalInfoOpen(false)}
            >
              Понятно
            </button>
          </div>
      ...
      >
        <div className={styles.originalInfoBody}>
          <p>Все товары со значком «ОРИГИНАЛ» проходят через маркетплейс POIZON.</p>
          <p>
            Перед отправкой каждый товар проверяется на оригинальность по внутренней системе
            верификации и получает подтверждение в виде сертификата и пломбы.
          </p>
          <p>
            Если товар окажется неоригинальным, мы вернем деньги в двукратном размере и начислим
            баллы на сумму покупки.
          </p>
        </div>
      </BottomSheet>
