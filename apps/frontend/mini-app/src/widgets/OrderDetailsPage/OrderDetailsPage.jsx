  ...
  return (
    <div className={styles.infoRow}>
      ...
      <div className={styles.infoText}>
        ...
        {secondary ? <div className={styles.infoSecondary}>{secondary}</div> : null}
      </div>
    ...
    <div className={styles.productRow}>
      <div className={styles.productThumbWrap} aria-hidden="true">
        <img
          src={item?.src}
          alt=""
          className={item?.muted ? styles.productThumbMuted : styles.productThumb}
          loading="lazy"
        />
      </div>
      <div className={styles.productMeta}>
        ...
        <div className={styles.productSub}>
          Размер: <span>{item?.size ? `${item.size}` : '—'}</span> &nbsp; · Артикул:{' '}
          <span>{item?.article ? `${item.article}` : ''}</span>
        </div>
        {item?.priceRub ? (
  ...
  return (
    <section className={styles.timelineCard} aria-label="Статус доставки">
      {rows.map((row) => {
        ...
        const lineClass =
          row.connector === 'dark' ? styles.timelineLine : styles.timelineLineUpcoming;
              ...
              {isStage ? (
                <div className={styles.timelineHeader}>
                  <div className={styles.timelineTitle}>
                    {row.title}
                    {row.showCaret ? (
                      <img src="/icons/profile/Wrap.svg" aria-hidden="true" alt="icon" />
                    ) : null}
                  </div>
                  {row.date ? <div className={styles.timelineDate}>{row.date}</div> : null}
                </div>
              ) : (
                <div className={styles.timelineEventRow}>
                  <div className={styles.timelineEventLabel}>{row.label}</div>
                  {row.date ? <div className={styles.timelineEventDate}>{row.date}</div> : null}
                </div>
              )}
            </div>
          </div>
      ...
      {items.slice(0, 10).map((it, idx) => {
        ...
        return (
          <button
            key={it?.id ?? `${idx}`}
            type="button"
            className={selected ? styles.thumbPillSelected : styles.thumbPillUnselected}
...
export default function OrderDetailsPage({ id, variant }) {
  const params = useParams();
  const router = useRouter();
  const resolvedId = id ?? params?.id;

  // No backend Orders module (Spec §11) — the ghost endpoint currently returns `null`.
  // Once the module is ready, just wire `lib/store/api.js#getOrderById` to a real fetch.
  const { data: order } = useGetOrderByIdQuery(resolvedId, {
    skip: !resolvedId,
  });
  ...
  const canShowBarcode = resolvedVariant === 'pickup' && Boolean(order?.barcode?.code);
  ...
  return (
    <div className={`tg-viewport ${styles.page}`}>
      <Header title={order.orderDateTitle || 'Заказ'} subtitle={order.orderNumberShort} />
      ...
      <main className={styles.main}>
        <section className={styles.statusBlock}>
          ...
          {order.receivedAt ? <div className={styles.statusDate}>{order.receivedAt}</div> : null}
          {order.statusSubtitle || order.subtitle ? (
            <div className={styles.statusSubtitle}>{order.statusSubtitle || order.subtitle}</div>
          ) : null}
          {order.statusHint ? <div className={styles.statusHint}>{order.statusHint}</div> : null}
        </section>
        ...
        <section className={styles.infoCard}>
          ...
          <InfoRow
            icon={<img src="/icons/profile/personIcon.svg" alt="personIcon Icon" />}
        ...
        {totals ? (
          <section className={styles.totalsCard}>
            <div className={styles.totalRowTop}>
              ...
              <div className={styles.totalValue}>{formatRub(totals.totalRub)}</div>
            </div>

            <div className={styles.totalRow}>
              <div className={styles.totalLabelMuted}>Скидка</div>
              <div className={styles.totalValueMuted}>{formatRub(totals.discountRub)}</div>
            </div>
            ...
            {totals.promocode ? (
              <div className={styles.totalSubRow}>
                ...
                <div className={styles.totalSubRight}>{formatRub(totals.promocode.rub)}</div>
              </div>
            ) : null}
            ...
            <div className={styles.totalSubRow}>
              ...
              <div className={styles.totalSubRight}>{formatRub(totals.giftPointsRub)}</div>
            </div>

            <div className={styles.totalRow}>
              <div className={styles.totalLabelMuted}>Доставка</div>
              <div className={styles.totalValueMuted}>{formatRub(totals.shippingRub)}</div>
            </div>
            ...
            <div className={styles.totalSubRow}>
              ...
              <div className={styles.totalSubRight}>{formatRub(totals.shippingRub)}</div>
            </div>
            ...
            <div className={styles.totalRowBottom}>
              ...
              <div className={styles.totalFinalValue}>{formatRub(totals.finalRub)}</div>
            </div>
          </section>
        ) : null}
        ...
        <button type="button" className={styles.supportBtn}>
          {resolvedVariant === 'cancelled' ? 'Написать в поддержку' : 'Чат с поддержкой'}
        </button>
      </main>
