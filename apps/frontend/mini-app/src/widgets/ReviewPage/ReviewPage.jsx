...
// SSR-safe local data read: `useSyncExternalStore` returns `null` on the
// server via `getServerSnapshot()`, and reads the snapshot from
// localStorage on the client. `getSnapshot` must be referentially stable —
// otherwise cascading renders start. Hence we cache by the raw string.
...
export default function ReviewPage({ id }) {
  ...
  // Product info is read from a `localStorage` snapshot (the previous page —
  // `/profile/purchased` or order details — writes it). There is no backend
  // Reviews module (Spec §11). `null` during SSR, real value after mount —
  // we read inside `useEffect` to avoid a hydration mismatch.
