...
function Radio({ checked }) {
  return (
    <div className={checked ? styles.radioChecked : styles.radio} aria-hidden="true">
      {checked ? (
        <img src="/icons/profile/CheckMark.svg" alt="" className={styles.radioTick} />
      ) : null}
    </div>
  );
}
...
export default function CreateReturnPage() {
  ...
  // Draft comes from `localStorage` — `saveReturnDraft` is written on the
  // `/profile/returns` page when an order is selected. There is no backend
  // Returns module (Spec §11), so if there is no draft we redirect the
  // user back instead of showing a broken mock product.
  const product = draft?.product ?? null;

  useEffect(() => {
    if (!draft?.product) {
      router.replace('/profile/returns');
    }
  }, [draft, router]);
  ...
  const selectedReasonLabel = reasons.find((reason) => reason.id === selected)?.label ?? '';
  ...
  function handleFilesPicked(event) {
    ...
    setPhotos((prev) => {
      ...
      const slotsToFill = [...order.slice(startIndex), ...order.slice(0, startIndex)].filter(
        (slotId, idx, arr) => arr.indexOf(slotId) === idx
      );
      ...
      for (const slotId of slotsToFill) {
        ...
        const shouldWrite = slotId === active || nextState[slotId] == null || files.length > 1;
  ...
  function handlePrimaryAction() {
    ...
    const requestPayload = {
      ...
      nextText: 'Мы проверяем вашу заявку на возврат, пожалуйста, ожидайте решения.',
      product,
      reasonId: selected,
    ...
    try {
      ...
      localStorage.setItem(RETURN_REQUESTS_KEY, JSON.stringify([requestPayload, ...prev]));
    } catch {
      // ignore
    }
  ...
  const isSubmitDisabled =
    step === 'details' &&
    (comment.trim().length === 0 || photoSlots.some((slot) => !photos[slot.id]));
  ...
  if (!product) {
    return (
      <div className={`tg-viewport ${styles.page}`}>
        <Header title="Оформление возврата" />
      </div>
    );
  }
        ...
        <section className={styles.orderCard}>
          ...
          <div className={styles.productRow}>
            <div className={styles.thumb} aria-hidden="true">
              <img src={product.src} alt="" className={styles.thumbImg} loading="lazy" />
            </div>
            <div className={styles.productMeta}>
              <div className={styles.productName}>{product.name}</div>
              <div className={styles.productSub}>
                Размер: <span>{product.size}</span> · Артикул: <span>{product.article}</span>
              </div>
              <div className={styles.productPrice}>{formatRub(product.priceRub)}</div>
            </div>
          </div>
        </section>
