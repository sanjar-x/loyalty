...
function normalizeRequest(stored) {
  ...
  return {
    ...stored,
    product: stored.product && typeof stored.product === 'object' ? stored.product : null,
    photos:
...
function PhotoThumb({ url }) {
  const resolvedUrl = typeof url === 'string' ? url : typeof url === 'object' ? url?.url : null;
...
export default function ReturnRequestPage({ id }) {
  // Backend Returns module doesn't exist yet — the only source is the user's
  // previously submitted request stored in `localStorage`. If not found, we
  // show the "not found" empty state.
      ...
      <div className={`tg-viewport ${styles.page}`}>
        ...
        <main className={styles.main}>
          <section className={styles.card} role="status" aria-live="polite">
            ...
            <div className={styles.cardText}>Возможно, она была удалена или ещё не создана.</div>
          </section>
        </main>
        <Footer />
      </div>
  ...
  const nextText =
    request.nextText ?? 'Мы проверяем вашу заявку на возврат, пожалуйста, ожидайте решения.';
    ...
    <div className={`tg-viewport ${styles.page}`}>
      ...
      <main className={styles.main}>
        <section className={styles.card}>
          {request.statusText ? <div className={styles.pill}>{request.statusText}</div> : null}
