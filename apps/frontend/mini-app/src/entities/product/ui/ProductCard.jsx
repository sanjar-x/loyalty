...
import { QuickAddSheetSlot } from './sheetSlots';
...
export default memo(
  function ProductCard({
    ...
    const productHref = productSlug ? `/product/${encodeURIComponent(productSlug)}` : '';

    // Client-side navigation — Next.js Router adds the PDP page in-memory.
    // Back button works instantly; RTK cache and Zustand state are preserved.
    // PDP data is fetched on PDP page mount after the tap (~600-800ms loading
    // state); list pages do no additional product-detail fetches (avoiding the
    // N+1 antipattern).
    ...
    const deliveryLabel =
      (typeof product?.deliveryDate === 'string' && product.deliveryDate.trim()) ||
      (typeof product?.deliveryText === 'string' && product.deliveryText.trim()) ||
      'Доставка';
      ...
      >
              ...
              <Image
                ...
                onError={() => {
                  setImgState((prev) => {
                    ...
                    if (typeof next === 'string' && next && next !== current.imgSrc) {
                      return {
                        productKey,
                        imgSrc: next,
        ...
        {variant === 'normal' &&
          ...
          (() => {
            ...
            const activeVisibleIdx =
              total <= maxDots
                ? carouselIdx
                : Math.min(maxDots - 1, Math.max(0, carouselIdx - Math.max(0, total - maxDots)));
        ...
        {/* Delivery button — opens the quick-add sheet */}
        <button
          type="button"
        ...
        {/* Mount the sheet only after it has been opened the first time, so
          we don't pay for the portal and RTKQ subscription inside every card. */}
        {isQuickAddOpen ? (
          <QuickAddSheetSlot
            product={product}
  ...
  (prev, next) => {
    ...
    // Also check for gallery changes (reference equality)
