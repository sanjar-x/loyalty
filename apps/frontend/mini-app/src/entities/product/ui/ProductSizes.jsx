...
export default function ProductSizes({
  ...
  // Desktop swipe — the hook auto-disables on touch.
  const sizesRowRef = useDragToScroll();
  ...
  return (
    <div className={cx(styles.c1, isDark && styles.dark)}>
      <div className={cx(styles.c2, styles.tw1)}>
        ...
        <div ref={sizesRowRef} className={cx(styles.c5, styles.tw3, isDark && styles.sizesRowDark)}>
          {sizes.map((size) => {
        ...
        >
          <div className={styles.sheetBody}>
            <img
              src={sizeGuideUrl || '/img/sizes.png'}
              alt="Размерная сетка"
              style={{ width: '100%', height: 'auto' }}
            />

            <p className={styles.sizeHint}>
              (Из-за различий в методах измерения единица измерения плоскости СМ может иметь
              погрешность в 2–3 см)
            </p>
          </div>
        </BottomSheet>
      </div>
