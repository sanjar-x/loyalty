...
// FSD: entities/* MUST NOT import @/app/providers/store (cycle).
// Thin codegen import + local default-params shim.
import { useStorefrontListBrandsApiV1StorefrontBrandsGetQuery } from '@/shared/api/codegen/api';
const useGetBrandsQuery = (arg, opts) =>
  useStorefrontListBrandsApiV1StorefrontBrandsGetQuery({ limit: 200, ...(arg || {}) }, opts);
            ...
            <div className={styles.list}>
              {group.brands.map((brand, brandIndex) => (
                <div key={brand.id}>
                  ...
                  {brandIndex < group.brands.length - 1 && <div className={styles.divider} />}
                </div>
              ))}
            </div>
          </div>
