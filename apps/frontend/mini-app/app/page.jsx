...
import { useGetTrendingProductsQuery, useCreateSearchHistoryMutation } from '@/lib/store/api';
...
import { normalizeSearchText } from '@/lib/home/utils';
import { useHomeSearch } from '@/lib/home/useHomeSearch';
import { useHomeFilters } from '@/lib/home/useHomeFilters';
import { useForYouFeed } from '@/lib/home/useForYouFeed';
import { useFilteredProducts } from '@/lib/home/useFilteredProducts';

import FilterChipsBar from './_home/FilterChipsBar';
import HomeFeed from './_home/HomeFeed';
import styles from './page.module.css';

/**
 * Bosh sahifa (`/`) — katalog/qidiruv/filtr.
 *
 * Audit #1 (god-komponentlar dekompozitsiyasi): ilgari bitta 1370-satrli
 * fayl edi. Endi bu — tashkilotchi (orchestrator):
 *  • pure helper'lar  → `lib/home/utils.js`
 *  • qidiruv state    → `lib/home/useHomeSearch.js`
 *  • filtr domeni     → `lib/home/useHomeFilters.js`
 *  • "Для вас" feed   → `lib/home/useForYouFeed.js`
 *  • filtr natijalari → `lib/home/useFilteredProducts.js`
 *  • presentation     → `./_home/{FilterChipsBar,HomeFeed}.jsx`
 *
 * Sahifada qoladigan glue: sheet open holatlari (7), `commitSearch`,
 * `resetToHome`, `handleCategoryChange`, `handleBack` + registratsiya,
 * init-from-URL — bularning hammasi qidiruv VA filtr state'iga tegadi.
 */
...
function Home() {
  ...
  // "Только что купили" — trending (placeholder ko'rinishida).
  const {
    data: latestData,
  ...
  /* ── Search commit (qidiruv + filtr glue) ── */
  ...
  /* ── Category tab change (qidiruv + filtr glue) ── */
