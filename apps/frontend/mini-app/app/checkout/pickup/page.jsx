...
import PvzDetailSheet from '@/components/blocks/pickup/PvzDetailSheet';
import { useBackHandlerStore } from '@/lib/features/telegram/back-handler';
import { distanceKm } from '@/lib/checkout/geo';
import { usePvzData } from '@/lib/checkout/usePvzData';
import { usePvzUrlState } from '@/lib/checkout/usePvzUrlState';
import { useAddressSuggest } from '@/lib/checkout/useAddressSuggest';
import { useLeafletPvzMap } from '@/lib/checkout/useLeafletPvzMap';

import ProviderChips from './ProviderChips';
import PickupModeToggle from './PickupModeToggle';
import styles from './page.module.css';

/**
 * `/checkout/pickup` — PVZ tanlash sahifasi (map / list / search step'lar).
 *
 * Audit #1 (god-komponentlar dekompozitsiyasi): ilgari bitta 1493-satrli
 * fayl edi. Endi bu — tashkilotchi:
 *  • pure helper'lar    → `lib/checkout/{geo,pvzProviders,pvzMarkerIcon}.js`
 *  • URL state          → `lib/checkout/usePvzUrlState.js`
 *  • PVZ data           → `lib/checkout/usePvzData.js`
 *  • Manzil suggest     → `lib/checkout/useAddressSuggest.js`
 *  • Leaflet imperative → `lib/checkout/useLeafletPvzMap.js` (eng zich qism)
 *  • presentation       → `./{ProviderChips,PickupModeToggle}.jsx`
 *
 * Marker click ↔ routing tsikli: `useLeafletPvzMap` `onMarkerClickRef`'ni qabul
 * qiladi; biz `selectPvzOnMap`'ni shu ref'ga effekt orqali yozamiz — hook'ning
 * `stopUserTracking`'sini sahifa glue'i ishlatishi mumkin.
 */
...
function CheckoutPickupPageInner() {
  ...
  // Marker click → routing tsiklini sindirish uchun ref (yuqoridagi izoh).
  ...
  // 500 PVZ × text-typing — sezilarli yutuq.
  ...
  /* ── Page glue: routing + PVZ tanlash ── */

  // CHK-016 Bug #2: PVZ tanlanganda lat/lon URL'da SAQLAYMIZ — refresh/back
  // paytida xarita to'g'ri markazga ochiladi va viewport-driven query PVZ
  // atrofini darhol qayta yuklab oladi.
    ...
    (pvzId) => {
      ...
      replacePickupUrl((params) => {
        ...
        if (selectedPoint?.lat != null && selectedPoint?.lon != null) {
          ...
          // Tanlangan PVZ atrofida kichik radius — diqqat uning markazida
          params.set('radius', '10');
        }
        // address'ni saqlaymiz — user qayerda turganini ko'rsatish uchun
      });
    },
    [pvzPointById, replacePickupUrl, stopUserTracking, setStep]
  );

  // `useLeafletPvzMap` marker click handler'iga `selectPvzOnMap`'ni ref orqali
  // beradi — hook ↔ sahifa tsiklini sindiradi (hook `stopUserTracking`'ni
  // ekspoz qiladi va `selectPvzOnMap` uni chaqiradi).
  useEffect(() => {
    onMarkerClickRef.current = selectPvzOnMap;
  }, [selectPvzOnMap]);

  // CHK-016 Bug #7: modal yopilsa pvzId URL'dan o'chirilsin.
  const closePvzModal = useCallback(() => {
    setIsPvzModalOpen(false);
    replacePickupUrl((params) => {
      params.delete('pvzId');
      // lat/lon saqlaymiz — user shu hududda turibdi
    });
  }, [replacePickupUrl]);

  // CHK-019: step-aware "PVZ tanlash" intent.
  //  • map step — eski selectPvzOnMap (lat/lon URL'ga yoziladi va modal)
  //  • list step — modal-only (map'ga sakramaydi, step saqlanadi)
  ...
  // CHK-019: yagona "Доставить сюда" handler.
  ...
  // CHK-016 Bug #1/#5: step-based back. Telegram BackButton va Header close
  // tugmasi shu intent'ni chaqiradi — history pop o'rniga mantiqiy "bir qadam".
  const onPickupBack = useCallback(() => {
    if (step === 'map' || step === 'list') {
      replacePickupUrl((params) => {
        params.set('step', 'search');
        params.delete('pvzId');
        // lat/lon saqlaymiz — search'dan keyin user yana mapga kelishi mumkin
      });
      setStep('search');
      return;
    }
    ...
    return (
      <div className={styles.c16}>
        ...
        <div className={cn(styles.c17, styles.tw10)}>
          ...
          {/* CHK-016 Bug #4: viewportArgs null bo'lsa user'ga prompt. */}
          {!viewportArgs ? (
          ...
          {/* Per-provider degradation: bitta provider degraded bo'lsa,
              qolgan provider markerlari hali ham ko'rinadi. */}
  ...
  return (
    <div className={styles.c74}>
        ...
        {filteredPvz.map((p, idx) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                // CHK-019: list step'da map'ga sakramaymiz — detail sheet
                // ochiladi, step saqlanadi.
                openPvzDetail(p.id);
              }}
              className={cn(
                styles.pvzListItem,
      ...
      {/* CHK-019: list step'da ham detail sheet — map'ga sakramaymiz */}
      <PvzDetailSheet
        open={isPvzModalOpen && Boolean(selectedPvz)}
        point={selectedPvz}
        onClose={closePvzModal}
