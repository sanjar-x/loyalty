...
/**
 * Pay tugmasi + xavfsizlik izohi + huquqiy matn (оферта / соглашение / ПДн).
 * Audit #1: `checkout/page.jsx` god-komponentidan ajratilgan presentation
 * komponent.
 *
 * Pay tugmasi har doim enabled — pre-flight qaror (`decideCheckoutAction`)
 * sahifadagi `onPay` handler ichida; bu komponent faqat busy/disabled
 * ko'rinishni render qiladi.
 *
 * @param {object} props
 * @param {boolean} props.isBusy            INITIATING|CONFIRMING — "Создаём заказ…"
 * @param {boolean} props.canPlaceOrder     enabled vs disabled vizual holat
 * @param {string}  props.payButtonTitle    "Оплатить через СБП" / "Оплатить картой"
 * @param {string}  props.paymentMethod     "sbp" | "card"
 * @param {string}  props.payButtonSuffix   ixtiyoriy suffix (hozircha "")
