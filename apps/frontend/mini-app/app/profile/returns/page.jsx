    ...
    >
      ...
      <div className={styles.productRow}>
        ...
        <div className={styles.productMeta}>
          ...
          <div className={styles.productPrice}>{formatRub(req.product.priceRub)}</div>
        </div>
      </div>
    </section>
  );
...
function ReturnOrderCard({ order }) {
  ...
  return (
    <section
      className={`${styles.card} ${styles.returnCard} ${order.disabled ? styles.cardDisabled : ''}`}
      role="button"
      tabIndex={0}
      onClick={() => {
    ...
    >
      <div className={styles.returnTopRow}>
        ...
        {order.pill ? <div className={styles.returnPill}>{order.pill}</div> : null}
      </div>
          ...
          <div key={`${it.article}_${idx}`}>
            ...
            <div className={styles.returnProductRow}>
              <div className={styles.returnThumb} aria-hidden="true">
                <img src={it.src} alt="" className={styles.returnThumbImg} loading="lazy" />
              </div>
              <div className={styles.returnProductMeta}>
                ...
                <div className={styles.returnProductSub}>
                  Размер: <span>{it.size}</span> · Артикул: <span>{it.article}</span>
                </div>
                <div className={styles.returnProductPrice}>{formatRub(it.priceRub)}</div>
              </div>
            </div>
          </div>
        ))}
...
export default function ReturnsPage() {
  ...
  // Backend Returns moduli yo'q (Spec §11) — bo'sh ro'yxat bilan boshlaymiz.
  // Modul tayyor bo'lganda `useReturnableOrdersQuery` orqali ulanadi.
  const returnOrders = useMemo(() => [], []);

  return (
    <div className={`tg-viewport ${styles.page}`}>
      <Header title="Возвраты" />
      <header className={styles.header}>
        <div className={styles.tabsWrap} role="tablist" aria-label="Разделы">
          <button
            ...
            className={`${styles.tabBtn} ${tab === 'requests' ? styles.tabBtnActive : ''}`}
          ...
          <button
            ...
            className={`${styles.tabBtn} ${tab === 'return' ? styles.tabBtnActive : ''}`}
      ...
      <main className={styles.main}>
        {tab === 'requests' ? (
          ...
          <section className={styles.emptyStateCard}>
            <div className={styles.emptyStateTitle}>Нет товаров для возврата</div>
            <div className={styles.emptyStateText}>
              Полученные заказы, которые можно вернуть, будут отображаться здесь
            </div>
          </section>
        )}
