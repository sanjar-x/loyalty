...
export default function SettingsPage() {
  ...
  // Telegram initData'dagi user — `me` (backend profil) kelguncha fallback.
  const { user: tgUnsafeUser } = useTelegram();

  const firstName =
    asNonEmptyTrimmedString(me?.first_name) || asNonEmptyTrimmedString(tgUnsafeUser?.first_name);
  const lastName =
    asNonEmptyTrimmedString(me?.last_name) || asNonEmptyTrimmedString(tgUnsafeUser?.last_name);
  const fullName = [firstName, lastName].filter(Boolean).join(' ') || null;

  const username =
    asNonEmptyTrimmedString(me?.username) || asNonEmptyTrimmedString(tgUnsafeUser?.username);
  ...
  const displayName = fullName || username || tgId || (isMePending ? '' : 'Пользователь');
  ...
  useEffect(() => {
    if (!isPickupSheetOpen) return;
    const frame = requestAnimationFrame(() => {
      setPickupDraftId(pickupPointId);
    });
    return () => cancelAnimationFrame(frame);
  }, [pickupPointId, isPickupSheetOpen]);

  useEffect(() => {
    if (!isRecipientSheetOpen) return;
    const frame = requestAnimationFrame(() => {
      setRecipientDraft({
        fio: recipient?.fio || '',
    ...
    return () => cancelAnimationFrame(frame);
  }, [recipient, isRecipientSheetOpen]);
  ...
  useEffect(() => {
    ...
    const fio = String(
      me?.fio ??
        ...
        [tgUnsafeUser?.first_name, tgUnsafeUser?.last_name].filter(Boolean).join(' ') ??
        ''
    ).trim();

    const phone = String(
      me?.phone ?? me?.phone_number ?? me?.tel ?? me?.mobile ?? me?.user_phone ?? ''
    ).trim();
  ...
  const recipientErrors = useMemo(() => {
    ...
    return {
      ...
      phone: phone.trim() && !isValidPhone(phone) ? 'Укажите в формате +7 XXX XXX-XX-XX' : '',
  ...
  return (
    <div className={styles.page}>
            ...
            >
              <div className={styles.rowLeft}>
                ...
                <div className={styles.rowText}>
                  ...
                  <div className={styles.rowValue}>{selectedPickup?.address || 'Не выбран'}</div>
                </div>
              </div>
            ...
            >
              <div className={styles.rowLeft}>
                ...
                <div className={styles.rowText}>
                  ...
                  <div className={styles.rowValue}>{recipientLabel || 'Не указан'}</div>
                </div>
              </div>
      ...
      <BottomSheet
        open={isPickupSheetOpen}
        onClose={() => setIsPickupSheetOpen(false)}
        title="Пункт выдачи"
        footer={
          <Button type="button" className={styles.sheetSaveBtn} onClick={savePickup}>
            Сохранить
          </Button>
        }
      >
        <div className={styles.sheetList} role="radiogroup" aria-label="Пункты выдачи">
          {pickupPoints.map((p) => {
      ...
      <BottomSheet
        open={isRecipientSheetOpen}
        onClose={() => setIsRecipientSheetOpen(false)}
        title="Получатель"
        footer={
          <Button type="button" className={styles.sheetSaveBtn} onClick={saveRecipient}>
            Сохранить
          </Button>
        }
      >
        <div className={styles.recipientSheet} aria-label="Данные получателя">
          <div className={styles.recipientHint}>
            ...
            <div className={styles.recipientHintText}>
              <div className={styles.recipientHintTitle}>Указывайте настоящие данные</div>
          ...
          <div className={styles.recipientFields}>
            <label className={styles.field}>
              ...
              <input
                ...
                onBlur={() => setRecipientTouched((prev) => ({ ...prev, fio: true }))}
                placeholder="ФИО"
                autoComplete="name"
              />
            ...
            <label className={styles.field}>
              ...
              <input
                ...
                onBlur={() => setRecipientTouched((prev) => ({ ...prev, phone: true }))}
                placeholder="Телефон"
                inputMode="tel"
                autoComplete="tel"
              />
            ...
            <label className={styles.field}>
              ...
              <input
                ...
                onBlur={() => setRecipientTouched((prev) => ({ ...prev, email: true }))}
