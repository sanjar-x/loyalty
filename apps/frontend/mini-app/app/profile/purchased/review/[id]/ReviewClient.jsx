...
// SSR-safe lokal ma'lumotni o'qish: `useSyncExternalStore` server'da
// `getServerSnapshot()` orqali `null` qaytaradi, klient'da localStorage'dan
// snapshot oladi. `getSnapshot` referent-stable bo'lishi shart — aks holda
// cascading render boshlanadi. Shu sababli raw string bo'yicha cache.
function useProductSnapshot(pid) {
  const cacheRef = useRef({ lastRaw: null, lastValue: null });

  const getSnapshot = () => {
    if (!pid) return null;
    let raw = null;
    try {
      raw = localStorage.getItem(REVIEW_PRODUCTS_KEY);
    } catch {
      raw = null;
    }
    ...
    cacheRef.current.lastValue = value;
    return value;
  };
...
export default function ReviewClient({ id }) {
  ...
  // Tovar ma'lumoti `localStorage`'dagi snapshot'dan o'qiladi (oldingi sahifa
  // — `/profile/purchased` yoki order details — yozadi). Backend Reviews moduli
  // yo'q (Spec §11). SSR'da `null`, mount'dan keyin haqiqiy qiymat — hydration
  // mismatch'iga yo'l qo'ymaslik uchun `useEffect` ichida o'qiymiz.
  const product = useProductSnapshot(pid);
  ...
  return (
    <div className="tg-viewport">
      <Header title="Оцените товар" />
      <main className={styles.root}>
        <section className={styles.productRow}>
          <div className={styles.thumbWrap}>
            {resolvedImage ? (
              <img src={resolvedImage} alt={product?.name ?? ''} className={styles.thumb} />
            ) : (
          ...
          <div className={styles.productMeta}>
            ...
            {product?.size || product?.article ? (
              <div className={styles.productSub}>
                ...
                {product?.size && product?.article ? <>&nbsp; · </> : null}
                {product?.article ? (
                  <>
        ...
        <div className={styles.form}>
          <div className={styles.prosWrap} data-filled={pros ? 'true' : 'false'}>
          ...
          <div className={styles.fieldWrap} data-filled={cons ? 'true' : 'false'}>
          ...
          <div className={styles.fieldWrap} data-filled={comment ? 'true' : 'false'}>
