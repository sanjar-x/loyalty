...
│   ├── api/                 # api-client (server backendFetch), bff (CSRF + proxyToBackend factory + error helpers), client-fetch (browser apiClient + ApiError), server-cache (TTL), geo
...
2. **Server-side**: `app/api/` route handlers call the main backend via `backendFetch()` (`@/shared/api/api-client`) with a Bearer token extracted from the `access_token` cookie. Media endpoints live under `/api/v1/admin/media/*` (consolidated into the main backend in REC-028; the standalone image_backend service has been retired).
