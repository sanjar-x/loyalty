...
`app-bg`, `app-bg-soft`, `app-panel`, `app-border`, `app-border-soft`, `app-text`, `app-text-dark`, `app-text-darker`, `app-text-secondary`, `app-muted`, `app-sidebar`, `app-sidebar-text`, `app-sidebar-soft`, `app-success`, `app-danger`, `app-accent`, `app-accent-danger`, `app-card`, `app-divider`, `app-divider-strong`, `app-code-bg`, `app-badge-china`, `app-badge-china-text`.

### Shared `ui/styles/<slice>.module.css`

Некоторые слайсы держат единый CSS-module с разделёнными классами по компонентам в `ui/styles/<slice>.module.css` (например, `entities/staff/ui/styles/staff.module.css`). Это допустимое отклонение от «один компонент = один module.css» — применяется, когда компоненты слайса делят общую систему классов (badge tones, table grids), и дублирование Tailwind-utility было бы шумом. Если слайс маленький и компоненты независимы — держи `*.module.css` рядом с компонентом.
