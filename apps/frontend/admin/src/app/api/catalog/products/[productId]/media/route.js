...
/**
 * Resolve a media asset URL from main-backend fields, falling back to a
 * direct lookup on /api/v1/admin/media/{storageObjectId} when the catalog
 * response does not yet carry the resolved url/imageVariants.
 */
async function resolveMediaUrl(asset, token) {
  ...
  if (asset.storageObjectId) {
    try {
      const { ok, data } = await backendFetch(
        `/api/v1/admin/media/${asset.storageObjectId}`,
...
export async function GET(request, { params }) {
  ...
  const resolved = await Promise.all(
    items.map(async (asset) => ({
      ...asset,
      _resolvedUrl: await resolveMediaUrl(asset, token),
    })),
  );
...
export async function POST(request, { params }) {
  ...
  if (ok) {
    // The list-page enrichment caches the thumbnail per productId — drop it
    // so the next /admin/products GET picks up the new asset.
