...
import { getOrFetch } from '@/shared/api/serverCache';
...
// Media-by-productId is part of the list-page enrichment N+1 (one
// `media?limit=1` call per row). A 60-second cache absorbs the typical
// "user clicks back to /admin/products" pattern without serving stale
// thumbnails after a real edit.
...
async function fetchProductMedia(token, productId) {
  ...
  // Cache per productId for 60s — repeated /admin/products navigation,
  // tab flips, and pagination all hit this enrichment loop.
  //
  // Throw on !ok so getOrFetch deletes the cache entry — otherwise a single
  // backend 5xx would freeze thumbnails at empty for the full TTL even after
  // recovery. Caller below catches and degrades gracefully to no thumbnail.
  try {
    return await getOrFetch(
      ...
      async () => {
        ...
        if (!ok) {
          const err = new Error(`media list ${status}`);
          err.status = status;
          throw err;
        }
        return data?.items ?? [];
...
// Each fetcher throws on !ok so getOrFetch evicts the entry on transient
// failures instead of caching an empty list for the full TTL. The outer
// Promise.allSettled then degrades each slot independently.
async function loadBrands(token) {
  const { ok, data } = await backendFetch(
    '/api/v1/admin/catalog/brands?offset=0&limit=200',
    { headers: { Authorization: `Bearer ${token}` } },
  );
  if (!ok) throw new Error('brands lookup failed');
  return data?.items ?? [];
}

async function loadCategoryTree(token) {
  const { ok, data } = await backendFetch(
    '/api/v1/admin/catalog/categories/tree',
    { headers: { Authorization: `Bearer ${token}` } },
  );
  if (!ok) throw new Error('categories lookup failed');
...
async function loadSuppliers(token) {
  ...
  if (!ok) throw new Error('suppliers lookup failed');
  return data?.items ?? [];
}

async function fetchLookupData(token) {
  const [brandsR, categoryTreeR, suppliersR] = await Promise.allSettled([
    getOrFetch('catalog:brands', LOOKUP_TTL_MS, () => loadBrands(token)),
    getOrFetch('catalog:categories:tree', LOOKUP_TTL_MS, () =>
      loadCategoryTree(token),
    ),
    getOrFetch('catalog:suppliers', LOOKUP_TTL_MS, () => loadSuppliers(token)),
  ]);

  return {
    brands: brandsR.status === 'fulfilled' ? brandsR.value : [],
    categoryTree:
      categoryTreeR.status === 'fulfilled' ? categoryTreeR.value : [],
    suppliers: suppliersR.status === 'fulfilled' ? suppliersR.value : [],
  };
}
 ...
 * 2) Fallback: query /api/v1/admin/media/{id} by storageObjectId to get
 *    processed URLs when the catalog response did not pre-resolve them.
 */
async function resolveMainImageUrl(mediaItems, token) {
  ...
  // Step 2 — resolve from main backend by storageObjectId
  const storageId =
...
async function enrichProduct(
  ...
  const image = await resolveMainImageUrl(media, token);
  ...
  // Price from ProductResponse.minPrice (kopecks) with currency derived from
  // the first variant's defaultPrice (or first SKU's resolved/explicit price)
  // — CAT-002 dropped the legacy flat-currency field on the response.
  ...
  if (item?.variants?.length) {
    for (const v of item.variants) {
      if (v.defaultPrice?.amount != null) {
        if (minPrice == null) minPrice = v.defaultPrice.amount;
        currency = v.defaultPrice.currency ?? currency;
        break;
      }
      for (const sku of v.skus ?? []) {
        const p = sku.resolvedPrice ?? sku.price;
        if (p?.amount != null) {
          if (minPrice == null) minPrice = p.amount;
