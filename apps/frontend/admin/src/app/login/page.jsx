...
// Hints shown above the form when we arrive at /login?reason=...
// `reason` comes from the Edge proxy (auth code from the backend on a
// permanent refresh failure) or from AuthProvider (after a 401 on
// /api/auth/me). Keep the copy short — it sits above the input, not as
// an error toast.
const REASON_HINTS = {
  SESSION_EXPIRED: 'Сессия завершена из-за неактивности. Войдите снова.',
  SESSION_REVOKED: 'Сессия отозвана. Возможно, вы вышли с другого устройства.',
  REFRESH_TOKEN_REUSE:
    'Сессия скомпрометирована. Все устройства разлогинены, войдите снова.',
  TOKEN_VERSION_STALE:
    'Сессия инвалидирована (вероятно, сменён пароль). Войдите снова.',
  IDENTITY_INVALID: 'Учётная запись недоступна. Обратитесь к администратору.',
...
function LoginForm() {
  ...
  const reasonHint = reason ? REASON_HINTS[reason] : null;
    ...
    <div className="bg-app-bg flex min-h-screen items-center justify-center p-5">
      ...
      >
        ...
        {reasonHint && !error && (
          <div
            role="status"
            className="text-app-muted mb-4 rounded-lg bg-amber-50 px-4 py-3 text-sm"
          >
            {reasonHint}
          </div>
        )}
...
// useSearchParams() bails out of static prerender, which Next.js 16
// requires to sit behind an explicit Suspense boundary.
