...
export function proxyToBackend({
  ...
  return async function handler(request, ctx = {}) {
    ...
    // Honor 204 for empty bodies regardless of caller's `successStatus`
    // — RFC 7231 forbids a payload-bearing status on a no-content
    // response, and `successStatus: 201` without a body would produce a
    // non-standard `201 No Content` that breaks intermediaries.
    const isEmpty = data === null || data === undefined;
    const responseInit = {
      status: isEmpty ? 204 : (successStatus ?? status ?? 200),
