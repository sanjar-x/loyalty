...
export function ToastProvider({ children }) {
  ...
  // Track every active timer so we can clear them on unmount — otherwise a
  // route change with N pending toasts leaves 2N timers firing dispatch
  // against an unmounted reducer (no-op but spammy and a minor leak).
  ...
  const addToast = useCallback(
    (type, message, options = {}) => {
      ...
      if (duration > 0) {
        scheduleTimer(() => dispatch({ type: 'DISMISS', id }), duration);
        scheduleTimer(
  ...
  const dismiss = useCallback(
    (id) => {
      ...
      scheduleTimer(() => dispatch({ type: 'REMOVE', id }), 300);
    },
    [scheduleTimer],
  );
