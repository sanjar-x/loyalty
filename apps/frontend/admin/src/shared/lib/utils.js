...
// Long form used in detail panels & invitation expiry rows where the year
// disambiguates "12 марта 2025" vs the current year. List rows that need
// the year + time use `formatDateTimeLong`.
...
export function formatDateTimeLong(value, { fallback = '—' } = {}) {
  if (!value) return fallback;
  const m = dayjs(value);
