      ...
      <table className="w-full min-w-[720px] text-sm">
        ...
        <tbody>
          {variables.map((v) => (
            <tr
              key={v.variableId}
              className="border-app-card hover:bg-app-bg-soft border-b transition-colors"
            >
              ...
              <td className="text-app-text px-3 py-2.5">
                {i18n(v.name, v.code)}
              </td>
              <td className="px-3 py-2.5">
                <span className="bg-app-border-soft text-app-muted rounded-md px-2 py-0.5 text-xs font-medium">
                  {SCOPE_LABELS[v.scope] ?? v.scope}
                </span>
              </td>
              <td className="text-app-muted px-3 py-2.5">
                {TYPE_LABELS[v.dataType] ?? v.dataType}
              </td>
              <td className="text-app-muted px-3 py-2.5 font-mono text-xs">
                {v.unit}
              </td>
              ...
              <td className="px-3 py-2.5 text-right">
                <div className="flex items-center justify-end gap-1">
                  <button
                    onClick={() => onEdit(v)}
                    className="text-app-muted hover:bg-app-card hover:text-app-text rounded-lg p-1.5 transition-colors"
                    title="Редактировать"
                  >
                    <svg
                      className="h-4 w-4"
                  ...
                  <button
                    onClick={() => onDelete(v)}
                    className="text-app-muted rounded-lg p-1.5 transition-colors hover:bg-red-50 hover:text-red-500"
                    title="Удалить"
                  >
                    <svg
                      className="h-4 w-4"
