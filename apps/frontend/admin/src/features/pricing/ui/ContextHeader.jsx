...
import { getGlobalValues } from '@/features/pricing/api/contexts';
...
export function ContextHeader() {
  ...
  const fetchFxData = useCallback(async () => {
    ...
    try {
      ...
      setFxError(err?.message || 'Не удалось загрузить курсы валют');
    }
  }, [contextId]);
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        {fxError && (
          <span
            className="bg-app-badge-china text-app-badge-china-text rounded-md px-2 py-0.5 text-xs"
            role="status"
            title={fxError}
          >
            FX недоступен
        ...
        {ctx.isFrozen && ctx.freezeReason && (
          <span className="text-app-muted text-xs" title={ctx.freezeReason}>
            —{' '}
      ...
      <div className="text-app-muted flex items-center gap-2 text-xs">
        <span>
          Код: <code className="font-mono">{ctx.code}</code>
        </span>
        {ctx.marginFloorPct != null && (
          <span>
            · Margin floor: {(Number(ctx.marginFloorPct) * 100).toFixed(1)}%
          </span>
        )}
