  ...
  return (
    <div className="relative">
      <input
        ...
        className={cn(
          'text-app-text h-9 w-full rounded-md border border-transparent bg-transparent px-2 font-mono text-sm transition-colors outline-none',
      ...
      {showSuggestions && filteredVars.length > 0 && (
        <div className="border-app-border absolute top-full left-0 z-20 mt-1 max-h-48 w-72 overflow-y-auto rounded-xl border bg-white p-1 shadow-lg">
          {filteredVars.slice(0, 15).map((v) => (
            <button
              ...
              className="hover:bg-app-card flex w-full items-center gap-2 rounded-lg px-3 py-1.5 text-left transition-colors"
            >
              <code className="text-app-text text-xs font-medium">
                {v.code}
              </code>
              <span className="text-app-muted truncate text-xs">
                {v.unit} · {v.scope}
              </span>
            </button>
          ))}
