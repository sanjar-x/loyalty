  ...
  return (
    <div
      className={cn(
        ...
        'hover:bg-app-bg-soft hover:border-gray-200',
    ...
    >
      <div className="flex shrink-0 flex-col gap-0.5 pt-2">
        <div className="flex items-center gap-1.5">
          <input
            ...
            className={cn(
              'border-app-border h-8 w-44 rounded-lg border bg-white px-2.5 text-sm transition-colors outline-none',
        ...
        {!isFinalPrice && binding.name && (
          <span
            className="pl-1 font-mono text-[10px] text-gray-400"
            title="Техническое имя в формуле"
          >
            {binding.name}
          </span>
        )}
      ...
      <div className="flex shrink-0 items-center gap-1.5 pt-2">
        <select
          ...
          disabled={readOnly}
          className={cn(
            'border-app-border h-8 rounded-lg border bg-white px-2 text-xs outline-none',
            'disabled:border-transparent disabled:bg-transparent',
            binding.component_tag === 'final_price' &&
        ...
        {!readOnly && (
          <div className="flex items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">
            {index > 0 && (
              <button
                onClick={onMoveUp}
                className="hover:text-app-text rounded p-1 text-gray-400"
                title="Вверх"
              >
                <svg
...
function labelToName(label) {
  ...
  return (
    result.replace(/_+/g, '_').replace(/^_|_$/g, '').slice(0, 64) || 'binding'
  );
}
