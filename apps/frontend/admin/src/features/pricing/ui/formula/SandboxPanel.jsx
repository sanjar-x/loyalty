...
export function SandboxPanel() {
  ...
  const runPreview = useCallback(async () => {
    ...
    try {
      ...
      // The shared apiClient throws a flat ApiError ({message, code, status,
      // details}); previewPrice has no other call sites, so err.message is
      // the canonical accessor. The previously-written deep-envelope path
      // `err?.data?.error?.message` was dead code (no consumer ever set
      // err.data) and was removed in REC-014.
  ...
  return (
    <div className="border-app-border bg-app-bg-soft flex flex-col gap-4 rounded-xl border p-4">
      ...
      {loading && (
        ...
        >
          <span
            className="border-app-muted inline-block h-3 w-3 animate-spin rounded-full border-2 border-t-transparent"
            aria-hidden="true"
          />
          Расчёт…
        </div>
...
function SandboxField({ label, value, onChange, placeholder }) {
  return (
    <label className="flex flex-col gap-0.5">
      <span className="text-app-muted text-[11px] font-medium">{label}</span>
      <input
        type="text"
        value={value}
