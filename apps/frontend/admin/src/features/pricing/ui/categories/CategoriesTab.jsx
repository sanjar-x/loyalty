...
import {
  getCategorySettings,
  upsertCategorySettings,
  deleteCategorySettings,
} from '@/features/pricing/api/categorySettings';
import { listVariables } from '@/features/pricing/api/variables';
...
export function CategoriesTab() {
  ...
  const fetchTree = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        setError(`Не удалось загрузить дерево категорий (${res.status})`);
        return;
      }
      setTree(await res.json());
    } catch (err) {
      setError(err.message || 'Не удалось загрузить дерево категорий');
    }
  }, []);
  ...
  useEffect(() => {
    Promise.all([fetchTree(), fetchVariables()]).finally(() =>
      setLoading(false),
    );
  ...
  async function handleDelete() {
    if (!confirm('Удалить настройки ценообразования для этой категории?'))
      return;
    setError(null);
    try {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-[260px_minmax(0,1fr)] gap-4">
        <div
          className="border-app-border bg-app-bg-soft flex flex-col gap-1 overflow-y-auto rounded-xl border p-2"
    ...
    <div key={node.id}>
      <button
        ...
        className={`flex w-full items-center rounded-lg px-3 py-1.5 text-left text-sm transition-colors ${
          selectedId === node.id
            ? 'text-app-text ring-app-border bg-white font-medium shadow-sm ring-1'
            : 'text-app-text hover:bg-white/60'
        }`}
        style={{ paddingLeft: level * 20 + 12 }}
      >
        {node.children?.length > 0 && (
          <span className="text-app-muted mr-1.5 text-xs">▸</span>
        )}
