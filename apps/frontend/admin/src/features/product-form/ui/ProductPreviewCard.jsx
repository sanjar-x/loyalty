...
// Same resolution rule the gallery thumbnail follows: derived
// no-background URL wins once bg-removal completes AND the active
// variant for the image is 'no-background' (default after a
// successful derivation). Falls back to the upload preview, then the
// stored URL.
function resolveImageSrc(image, uploads, bgState, bgVariant) {
  if (
    bgState?.status === 'completed' &&
    bgVariant === 'no-background' &&
    bgState.derivedUrl
  ) {
    return bgState.derivedUrl;
  }
  return uploads?.[image.localId]?.url || image.url;
}
                ...
                images.map((img) => (
                  <img
                    ...
                    src={resolveImageSrc(
                      img,
                      uploads,
                      bgRemovalStateByLocalId[img.localId],
                      bgVariantByLocalId[img.localId] ?? 'original',
                    )}
                    alt=""
