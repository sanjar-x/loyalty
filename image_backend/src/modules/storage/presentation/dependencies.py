...
class StorageProvider(Provider):
    ...
    @provide(scope=Scope.APP)
    def sse_manager(self, redis_client: aioredis.Redis) -> SSEManager:
        """Create an SSE manager backed by the app-scoped Redis client.

        Args:
            redis_client: The async Redis client from ``CacheProvider``.

        Returns:
            An ``SSEManager`` for publishing/subscribing processing status.
        """
