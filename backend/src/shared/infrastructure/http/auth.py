"""
Base authentication managers for HTTP provider clients (REC-033).
...
handle token acquisition, caching, and refresh. Promoted from
``logistics/infrastructure/providers/base_auth.py`` so payment /
notification / future external integrations can use the same primitives.
"""
...
from src.shared.infrastructure.http.errors import ProviderAuthError
