...
class DomainEvent:
    ...
    # ``abstract`` is read out of ``**kwargs`` rather than declared as a
    # named keyword so that pre-existing module subclasses whose own
    # ``__init_subclass__`` forwards ``**kwargs: object`` keep
    # type-checking. Once those subclasses migrate to
    # :class:`ModuleDomainEvent` in PR-4, this can be tightened back
    # into a typed keyword parameter.
    def __init_subclass__(cls, **kwargs: Any) -> None:
        abstract = bool(kwargs.pop("abstract", False))
