...
class StateMachineMixin(Generic[StateT]):
    ...
    * ``_already_terminal_exc`` — exception class raised when a move
    ...
    _ALLOWED_TRANSITIONS: ClassVar[Mapping[StateT, frozenset[StateT]]]  # type: ignore[misc]
    _TERMINAL_STATES: ClassVar[frozenset[StateT]]  # type: ignore[misc]
    _invalid_transition_exc: ClassVar[type[Exception]]
    _already_terminal_exc: ClassVar[type[Exception]]
