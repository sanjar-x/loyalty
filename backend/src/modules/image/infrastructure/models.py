"""Image module ORM models.

Defines the SQLAlchemy ORM model for the ``storage_objects`` table —
a centralized registry of every file in the system. Metadata is kept
in sync with the S3-compatible object storage. Ported from
``image_backend/src/modules/storage/infrastructure/models.py``;
class renamed to follow main-backend ``…Model`` convention
(see ``favorites/infrastructure/models.py`` for reference).
"""
