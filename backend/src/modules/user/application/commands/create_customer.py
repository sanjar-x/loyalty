...
Provides the customer creation workflow triggered by an
``IdentityRegisteredEvent`` (account_type=CUSTOMER) from the Identity
module via the outbox consumer. Also used as a self-healing fallback
from profile endpoints when the async event pipeline has not yet
created the customer record.

Referral attribution (``referred_by``, codes, loyalty wallet) lives in
the dedicated ``referral`` bounded context — Customer holds PII only.
"""
...
class CreateCustomerHandler:
    ...
    Idempotent: if a customer with the given ID already exists, creation
    is skipped. Race-safe: concurrent creation attempts are tolerated
