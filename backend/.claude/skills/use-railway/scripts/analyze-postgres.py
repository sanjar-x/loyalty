...
class AnalysisResult:
    ...
    )  # Raw unfiltered logs for LLM analysis
    recent_errors: list[str] = field(
        default_factory=list
    )  # Legacy: filtered error logs
    metrics_history: dict[str, Any] | None = (
        None  # Multi-window time series + trend analysis for CPU, memory, disk, network
    )
    collection_status: dict[str, dict[str, Any]] = field(
...
def run_psql_query_safe(
    service: str, query: str, timeout: int = 60
) -> tuple[int, str, str]:
...
def parse_batched_analysis(data: dict[str, Any], result: AnalysisResult) -> None:
    ...
    if mem_config:
        ...
        for row in mem_config:
            ...
            if unit == "8kB":
                ...
                result.memory_config[name] = {
                    "value": int(setting) if str(setting).isdigit() else 0,
                    "unit": unit,
                    "mb": round(mb, 1),
                }
            elif unit == "kB":
                ...
                result.memory_config[name] = {
                    "value": int(setting) if str(setting).isdigit() else 0,
                    "unit": unit,
                    "mb": int(setting) if str(setting).isdigit() else 0,
                }
                ...
                result.memory_config[name] = {
                    "value": int(setting) if str(setting).isdigit() else setting
                }
    ...
    result.connection_states = [
        {"state": t.get("state"), "count": str(t.get("count", 0))} for t in conn_states
    ]

    # Connections by app
    conn_app = data.get("connections_by_app", [])
    result.connections_by_app = [
        {"app": t.get("app", ""), "count": str(t.get("count", 0))} for t in conn_app
    ]

    # Connections by age
    conn_age = data.get("connections_by_age", [])
    result.connections_by_age = [
        {"range": t.get("range"), "count": str(t.get("count", 0))} for t in conn_age
    ]
    ...
    result.pg_stat_statements_installed = data.get(
...
def get_disk_usage(
    service: str, environment_id: str | None = None, service_id: str | None = None
) -> dict[str, Any] | None:
...
def get_cluster_logs(
    ha_cluster: dict[str, Any] | None, environment_id: str | None, limit: int = 100
) -> list[dict[str, Any]]:
        ...
        for edge in deployments:
            ...
            log_result = subprocess.run(
                [
                    api_script,
                    log_query,
                    json.dumps({"deploymentId": deployment_id, "limit": limit}),
                ],
                capture_output=True,
                text=True,
                timeout=30,
...
def analyze_postgres(
    ...
    def task_database_query():
        ...
        code, stdout, stderr = run_psql_query_safe(
            service, analysis_query, timeout=timeout
        )
        if code != 0:
            # Retry once — SSH sessions can drop mid-query
            if not quiet:
                print(
                    f"        Database query failed ({stderr or 'unknown'}), retrying...",
                    file=sys.stderr,
                    flush=True,
                )
            code, stdout, stderr = run_psql_query_safe(
    ...
    def task_ha_cluster():
        ...
        code, stdout, stderr = run_ssh_query(
            service, "curl -s localhost:8008/cluster 2>/dev/null || echo '{}'"
        )
    ...
    if ha_result == "skipped_not_ha":
        ...
        result.collection_status["ha_cluster"] = {
            ...
            "error": "Failed to retrieve Patroni cluster data"
            if is_ha_service
            else "not an HA service",
        }
    ...
    if skip_logs:
        ...
        if result.ha_cluster and environment_id:
            ...
            result.cluster_logs = get_cluster_logs(
                result.ha_cluster, environment_id, limit=5000
            )
    else:
        result.recent_logs = []
        result.collection_status["logs_api"] = {
...
def generate_recommendations(result: AnalysisResult) -> list[dict[str, str]]:
    ...
    if result.memory_config:
            ...
            if sb_mb < 128:
                ...
                if system_memory_gb:
                    ...
                    rec_sb = (
                        f"{rec_sb_mb}MB"
                        if rec_sb_mb < 1024
                        else f"{round(rec_sb_mb / 1024, 1)}GB"
                    )
                recommendations.append(
                    {
                        "priority": "immediate",
            ...
            if ec_mb < 512:
                ...
                if system_memory_gb:
                    ...
                    rec_ec = (
                        f"{rec_ec_mb}MB"
                        if rec_ec_mb < 1024
                        else f"{round(rec_ec_mb / 1024, 1)}GB"
                    )
                recommendations.append(
                    {
                        "priority": "short-term",
        ...
        if work_mem and work_mem.get("mb"):
            ...
            max_conns = (
                result.connections.get("max", 100) if result.connections else 100
            )
            rec_wm = "32MB"
            if system_memory_gb:
            ...
            if result.database_stats:
                ...
                temp_gb = (
                    round(temp_bytes / 1024 / 1024 / 1024, 1) if temp_bytes > 0 else 0
                )
    ...
    if result.cache_hit:
        ...
        if table_hit is not None and table_hit < 95:
            ...
            context = (
                f" Worst tables: {', '.join(worst_tables)}." if worst_tables else ""
            )
            recommendations.append(
                {
                    "priority": priority,
    ...
    if result.database_stats:
        ...
        if days_since_reset and days_since_reset > 0:
            ...
            if gb_per_day > 5:  # More than 5GB/day is concerning
                ...
                wm_mb = (
                    result.memory_config.get("work_mem", {}).get("mb", 4)
                    if result.memory_config
                    else 4
                )
                recommendations.append(
        ...
        recommendations.append(
            {
                ...
                "commands": [
                    f'DROP INDEX IF EXISTS "{idx["index"]}";  -- saves {idx["size"]}'
                    for idx in droppable_indexes[:3]
                ],
            }
        )
                    ...
                    recommendations.append(
                        {
                            ...
                            "commands": [
                                f"SELECT pg_cancel_backend({q.get('pid')});  -- Graceful cancel"
                            ],
                        }
                    )
            except ValueError, TypeError:
    ...
    if result.recent_errors and len(result.recent_errors) > 5:
        ...
        error_samples = [
            e[:60] if isinstance(e, str) else str(e)[:60]
            for e in result.recent_errors[:3]
        ]
        recommendations.append(
            {
                "priority": "short-term",
...
def format_report(result: AnalysisResult) -> str:
    ...
    if result.connections:
        ...
        lines.append(
            f"| - Active/Idle/IdleTxn | {active} / {idle} / {idle_in_txn} | {'Warning' if idle_in_txn > 5 else '-'} |"
        )
    ...
    if result.disk_usage:
        ...
        lines.append(
            f"| Disk | {result.disk_usage['used']} / {result.disk_usage['total']} ({pct}%) | {status} |"
        )
    ...
    if result.cache_hit:
        ...
        if index_hit is not None:
            status = (
                "Healthy" if index_hit >= 99 else "OK" if index_hit >= 95 else "Warning"
            )
    ...
    if result.xid_age:
        ...
        status = (
            "Critical" if millions > 150 else "Warning" if millions > 100 else "Healthy"
        )
    ...
    if result.cpu_memory:
        if "cpu_percent" in result.cpu_memory:
            ...
            if result.cpu_memory.get("cpu_limit"):
                lines.append(
                    f"| CPU Limit | {result.cpu_memory['cpu_limit']} vCPU | - |"
                )
        if "memory_gb" in result.cpu_memory:
            ...
            if result.cpu_memory.get("memory_limit_gb"):
                ...
                status = "-"
            lines.append(
                f"| Memory Usage | {mem} GB{utilization}{trend_str} | {status} |"
            )
    ...
    if result.database_stats:
        ...
        if (
            stats_reset
            and stats_reset != "unknown"
            and stats_reset != "never"
            and len(stats_reset) > 10
        ):
        ...
        if days_since_reset:
            ...
            if gb_per_day > 5:
                ...
                temp_status = "OK"
            lines.append(
                f"| Temp Files | {temp_files:,} ({temp_gb} GB) over {days_since_reset}d (~{files_per_day}/day, {gb_per_day} GB/day) | {temp_status} |"
            )
        else:
            lines.append(
                f"| Temp Files | {temp_files:,} ({temp_gb} GB since reset) | - |"
            )
    ...
    if result.memory_config:
        ...
        if "effective_cache_size" in mem:
            ...
            lines.append(
                f"| effective_cache_size | {mb} MB | 50-75% of RAM | {status} |"
            )
        ...
        if "max_parallel_workers_per_gather" in mem:
            ...
            status = "Disabled" if val == 0 else "OK"
            lines.append(
                f"| max_parallel_workers_per_gather | {val} | 2-4 | {status} |"
            )
    ...
    if result.cache_per_table:
        ...
        for t in result.cache_per_table[:10]:
            ...
            status = (
                "OK" if hit_pct >= 95 else "Warning" if hit_pct >= 80 else "Critical"
            )
            lines.append(
                f"| {t['table']} | {t['hit_pct']}% | {int(t['disk_reads']):,} | {status} |"
            )
        lines.append("")
    ...
    if result.table_sizes:
        ...
        for t in result.table_sizes[:10]:
            ...
            lines.append(
                f"| {full_name} | {t['size']} | {data_mb}MB | {index_mb}MB | {row_count} |"
            )
        lines.append("")
    ...
    if result.vacuum_health:
        ...
        lines.append("")
        lines.append(
            "| Schema.Table | Dead Rows | Dead % | V/AV Count | Last Analyze | XID Age | Flags |"
        )
        lines.append(
            "|--------------|-----------|--------|------------|--------------|---------|-------|"
        )
        for t in result.vacuum_health[:10]:
            ...
            xid_millions = (
                round(int(xid_age) / 1_000_000, 1) if xid_age.isdigit() else 0
            )
            flags = []
            ...
            full_name = f"{schema}.{table}" if schema != "public" else table
            lines.append(
                f"| {full_name} | {int(t['dead_rows']):,} | {t['dead_pct']}% | {vacuum_count}/{autovacuum_count} | {last_analyze} | {xid_millions}M | {flags_str} |"
            )
        lines.append("")
    ...
    if result.unused_indexes:
        ...
        for t in result.unused_indexes[:20]:
            ...
            lines.append(
                f"| {full_name} | {t['index']} | {t['size']} | {idx_type} | {table_idx_scans} |"
            )
        lines.append("")
    ...
    if result.invalid_indexes:
        ...
        for t in result.invalid_indexes:
            lines.append(
                f"| {t.get('schema', '')} | {t.get('table', '')} | {t.get('index', '')} |"
            )
        lines.append("")
    ...
    if result.top_queries:
        ...
        lines.append("")
        lines.append(
            "| Query | Calls | Total (min) | Mean (ms) | Min/Max (ms) | Stddev | Rows/Call | Cache Hit % | Temp R/W | Plan (ms) | I/O Time (ms) |"
        )
        lines.append(
            "|-------|-------|-------------|-----------|--------------|--------|-----------|-------------|----------|-----------|---------------|"
        )
        for t in result.top_queries[:15]:
            ...
            io_time = (
                f"{blk_read + blk_write:.0f}" if (blk_read + blk_write) > 0 else "-"
            )
            lines.append(
                f"| {query}... | {t.get('calls', '')} | {t.get('total_min', '')} | {t.get('mean_ms', '')} | {min_max} | {stddev} | {rows_per_call} | {cache_str} | {temp_str} | {plan_ms} | {io_time} |"
            )
        lines.append("")
    ...
    if result.long_running_queries:
        ...
        for q in result.long_running_queries:
            lines.append(
                f"| {q.get('pid', '')} | {q.get('duration_sec', '')} | {q.get('query', '')[:40]}... |"
            )
        lines.append("")
    ...
    if result.idle_in_transaction:
        ...
        for txn in result.idle_in_transaction:
            lines.append(
                f"| {txn.get('pid', '')} | {txn.get('idle_sec', '')} | {txn.get('user', '')} | {txn.get('app', '')[:15]} | {txn.get('last_query', '')[:30]}... |"
            )
        lines.append("")
    ...
    if result.blocked_queries:
        ...
        for q in result.blocked_queries:
            lines.append(
                f"| {q.get('pid', '')} | {q.get('wait_sec', '')} | {q.get('user', '')} | PID {q.get('blocking_pid', '')} | {q.get('query', '')[:30]}... |"
            )
        lines.append("")
    ...
    if result.locks:
        ...
        for lock in result.locks:
            lines.append(
                f"| {lock.get('locktype', '')} | {lock.get('mode', '')} | {lock.get('user', '')} | {lock.get('app', '')[:15]} | {lock.get('query', '')[:25]}... |"
            )
        lines.append("")
    ...
    if result.seq_scan_tables:
        ...
        for t in result.seq_scan_tables[:10]:
            lines.append(
                f"| {t.get('table', '')} | {t.get('seq_scans', '')} | {t.get('idx_scans', '')} | {t.get('rows', '')} |"
            )
        lines.append("")
    ...
    if result.replication:
        ...
        for r in result.replication:
            lines.append(
                f"| {r.get('client', '')} | {r.get('state', '')} | {r.get('sent_lsn', '')} | {r.get('replay_lsn', '')} |"
            )
        lines.append("")
    ...
    if result.ha_cluster:
        ...
        if members:
            ...
            for m in members:
                ...
                lines.append(
                    f"| {m.get('name', '')} | {m.get('role', '')} | {m.get('state', '')} | {m.get('timeline', '')} | {lag} |"
                )
        lines.append("")
    ...
    if result.bgwriter:
        ...
        lines.append("|--------|-------|")
        bg = result.bgwriter
        total_checkpoints = bg.get("checkpoints_timed", 0) + bg.get(
            "checkpoints_req", 0
        )
        timed_pct = (
            round(100.0 * bg.get("checkpoints_timed", 0) / total_checkpoints, 1)
            if total_checkpoints > 0
            else 0
        )
        lines.append(
            f"| Checkpoints (timed/requested) | {bg.get('checkpoints_timed', 0):,} / {bg.get('checkpoints_req', 0):,} ({timed_pct}% timed) |"
        )
        ...
        lines.append(
            f"| Buffers: backend fsync | {bg.get('buffers_backend_fsync', 0):,} |"
        )
    ...
    if result.progress_vacuum:
        ...
        for vac in result.progress_vacuum:
            ...
            lines.append(
                f"| {vac.get('pid', '')} | {vac.get('relname', '')} | {vac.get('phase', '')} | {pct}% ({scanned:,}/{total:,} blks) |"
            )
        lines.append("")
    ...
    if result.ssl_stats:
        ...
        ssl_pct = (
            round(100.0 * ssl.get("ssl_connections", 0) / total, 1) if total > 0 else 0
        )
        ...
        if versions:
            ...
            for v in versions:
                lines.append(
                    f"  - {v.get('version', 'unknown')}: {v.get('count', 0)} connections"
                )
        lines.append("")
