...
def extract_persistence(info: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "rdb_current_bgsave_time_sec": _safe_int(
            info.get("rdb_current_bgsave_time_sec")
        ),
...
def generate_recommendations(result: RedisAnalysisResult) -> list[dict[str, str]]:
    ...
    if result.cache:
        ...
        if (
            hit_rate < 80
            and (
                result.cache.get("keyspace_hits", 0)
                + result.cache.get("keyspace_misses", 0)
            )
            > 0
        ):
            recs.append({
...
def format_report(result: RedisAnalysisResult) -> str:
    ...
    if result.throughput:
        ...
        lines.append(
            f"| Total Commands | {_format_number(t.get('total_commands_processed', 0))} |"
        )
        lines.append(
...
def analyze_redis(
    ...
    progress(
        3,
        5,
        "Running analysis (Redis INFO, slowlog, bigkeys, metrics, logs in parallel)...",
        quiet,
    )
    ...
    def task_redis_info():
        ...
        if not ssh_available:
            return (
                "failed",
                f"SSH not available: {ssh_stderr or 'connection failed'}",
                "",
            )
    ...
    if info_status == "ok" and info_raw:
        ...
        result.collection_status["redis_info"] = {
            "status": "failed",
            "error": info_error,
        }
    ...
    if slg_status == "ok" and slg_raw:
        ...
        result.collection_status["slowlog_entries"] = {
            "status": "failed",
            "error": slg_error,
        }
    ...
    if metrics_result:
        ...
        result.collection_status["metrics"] = {
            "status": "failed",
    ...
    if not quiet:
        ...
        print(
            f"  Analysis complete{dal._progress_timer.total_elapsed()}",
            file=sys.stderr,
            flush=True,
        )
