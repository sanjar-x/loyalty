...
def run_railway_command(args: list[str], timeout: int = 30) -> tuple[int, str, str]:
    ...
    try:
        result = subprocess.run(
            ["railway"] + args, capture_output=True, text=True, timeout=timeout
        )
...
def run_psql_query(service: str, query: str, timeout: int = 60) -> tuple[int, str]:
    ...
    command = (
        f'''PAGER='' psql $DATABASE_URL -P pager=off -t -A -c "{query}" 2>/dev/null'''
    )
...
def get_all_metrics_from_api(
    ...
    start_date = (datetime.now(UTC) - timedelta(hours=hours)).isoformat()
    script_dir = os.path.dirname(os.path.abspath(__file__))
    api_script = os.path.join(script_dir, "railway-api.sh")
    ...
    try:
        ...
        if raw_series:
            combined["metrics_history"] = _build_metrics_history(
                raw_series, hours=hours
            )
...
def _build_metrics_history(
    ...
    for measurement, values in raw_series.items():
        ...
        if produce_short_window:
            ...
            recent_nums = [
                v["value"] for v in recent_values if v.get("value") is not None
            ]
...
def _trend_indicator(metrics_history: dict[str, Any] | None, metric_name: str) -> str:
    ...
    window_data = (
        windows.get("24h") or next(iter(windows.values()), None) if windows else None
    )
