...
[⚠ TODO] POST /products/{id}/media              → привязать media asset (endpoint не реализован)
...
| 422 | ValidationError          | `titleI18n` пустой, slug невалидный |

---

## Шаг 2. Загрузка медиа (ImageBackend)

> ⚠ **Статус реализации**: Steps 1–5 (ImageBackend) полностью реализованы. STEP 6 (привязка к продукту через catalog API) **ещё не реализован** — endpoint для создания/обновления media_assets в catalog-бэкенде отсутствует.
...
```
 ...
 └───┬─────────────────────┬──────────────────────┬──────────────────────┬───┘
     ...
     │                     │                      │                      │
 ┌───┴─────────────────────┴──────────────────────┴──────────────────────┴───┐
 │      STEP 6 — Привязать к продукту (⚠ endpoint не реализован)             │
 └───┬─────────────────────┬──────────────────────┬──────────────────────┬───┘
     │                     │                      │                      │
     │  ⚠ ВНИМАНИЕ: В текущей версии API нет endpoint для привязки       │
     │    media к продукту через REST. Поле `media` в POST/PATCH         │
     │    /products НЕ существует. Таблица media_assets есть в БД,       │
     │    но endpoint ещё не добавлен в роутер. Требуется реализация.    │
     │                     │                      │                      │
```
...
curl -s GET /api/v1/catalog/storefront/categories/$CATEGORY_ID/form-attributes?lang=ru \
