...
async def create_product(
    ...
    *,
    ...
    assert resp.status_code == 201, (
        f"create_product failed: {resp.status_code} {resp.text}"
    )
    return resp.json()
...
class TestProductEndpoints:
    ...
    async def test_get_product_success(
        ...
        resp = await admin_client.get(f"/api/v1/catalog/products/{created['id']}")
        assert resp.status_code == 200
        data = resp.json()
        for field in (
            "id",
    ...
    async def test_get_product_not_found_returns_404(
        ...
        resp = await admin_client.get(f"/api/v1/catalog/products/{uuid.uuid4()}")
        assert resp.status_code == 404
    ...
    async def test_delete_product_success(
        ...
        cat = await create_category(admin_client)
        created = await create_product(admin_client, brand["id"], cat["id"])
        resp = await admin_client.delete(f"/api/v1/catalog/products/{created['id']}")
