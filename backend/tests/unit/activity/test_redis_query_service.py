...
class _FakeRedis:
    def __init__(self, data: dict[str, list[tuple[bytes | str, float]]]) -> None:
        self._data = data
...
async def test_trending_weekly_returns_ranked_entities() -> None:
    ...
    service = RedisActivityQueryService(client, _FakeLogger())  # type: ignore[arg-type]
...
async def test_trending_category_uses_category_key() -> None:
    cid = uuid.uuid4()
    pid = uuid.uuid4()
    client = _FakeRedis({f"trending:category:{cid}": [(str(pid), 7.0)]})
    service = RedisActivityQueryService(client, _FakeLogger())  # type: ignore[arg-type]
...
async def test_returns_empty_on_redis_failure() -> None:
    logger = _FakeLogger()
    service = RedisActivityQueryService(_RaisingRedis(), logger)  # type: ignore[arg-type]
