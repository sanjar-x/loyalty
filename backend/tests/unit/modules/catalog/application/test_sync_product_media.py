...
def test_compute_diff_all_new():
    current = []
    incoming = [{"storage_object_id": str(uuid.uuid4()), "url": "https://cdn/a.webp", "role": "MAIN"}]
...
def test_compute_diff_update_changed_role():
    sid = str(uuid.uuid4())
    mid = str(uuid.uuid4())
    current = [{"storage_object_id": sid, "id": mid, "role": "GALLERY", "sort_order": 0, "variant_id": None}]
    incoming = [{"storage_object_id": sid, "role": "MAIN", "sort_order": 0, "variant_id": None}]
    to_add, to_update, to_delete = compute_media_diff(current, incoming)
...
def test_compute_diff_no_change():
    sid = str(uuid.uuid4())
    mid = str(uuid.uuid4())
    current = [{"storage_object_id": sid, "id": mid, "role": "MAIN", "sort_order": 0, "variant_id": None}]
    incoming = [{"storage_object_id": sid, "role": "MAIN", "sort_order": 0, "variant_id": None}]
    to_add, to_update, to_delete = compute_media_diff(current, incoming)
    assert len(to_add) == 0
    assert len(to_update) == 0
...
def test_compute_diff_external_urls():
    current = [{"url": "https://ext.com/old.jpg", "is_external": True, "id": str(uuid.uuid4())}]
