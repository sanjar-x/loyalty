      ...
      "attributes": [
        {
          ...
          "values": [
            { "id": "uuid", "code": "eu42", "slug": "eu42", "valueI18N": {"ru":"EU 42", "en":"EU 42"}, "metaData": {}, "sortOrder": 0 },
            { "id": "uuid", "code": "eu43", "slug": "eu43", "valueI18N": {"ru":"EU 43", "en":"EU 43"}, "metaData": {}, "sortOrder": 1 }
          ],
          "sortOrder": 1
        }
      ]
...
```
 ...
 └───┬─────────────────────┬──────────────────────┬──────────────────────┬───┘
     ...
     │                     │                      │                      │
 ┌───┴─────────────────────┴──────────────────────┴──────────────────────┴───┐
 │      STEP 5a — SSE-стриминг статуса (предпочтительно)                     │
 └───┬─────────────────────┬──────────────────────┬──────────────────────┬───┘
     │  GET /api/v1/media/{storageObjectId}/status│                      │
     │  ─────────────────────────────────────────►│                      │
     │  ◄── SSE stream ───────────────────────────│                      │
     │      event: status_update                  │                      │
     ...
     │                     │                      │                      │
 ┌───┴─────────────────────┴──────────────────────┴──────────────────────┴───┐
 │      STEP 5b — Polling метаданных (альтернатива)                          │
 └───┬─────────────────────┬──────────────────────┬──────────────────────┬───┘
     │  GET /api/v1/media/{storageObjectId}       │                      │
     │  ─────────────────────────────────────────►│                      │
     │  ◄── 200 { storageObjectId: "uuid",        │                      │
     │           status: "COMPLETED",              │                     │
...
### Авторизация ImageBackend

Все эндпоинты ImageBackend требуют API-ключ. Передаётся через header или query-параметр:

```
X-API-Key: <key>        # header (предпочтительно)
?api_key=<key>           # query parameter (альтернатива)
```

### Внешний импорт медиа

Импорт изображения по внешнему URL без загрузки файла (пропускает Steps 1-2):
...
{
  "storageObjectId": "uuid",
  "url": "https://cdn.../imported.webp",
  "variants": []
}
```

После импорта `storageObjectId` используется в Step 6 (привязка к продукту) как обычно.
...
curl -X POST /api/v1/media/upload \
  -H "X-API-Key: $IMAGE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"contentType":"image/jpeg","filename":"photo.jpg"}'
# → 201 {"presignedUrl":"https://s3.../...","storageObjectId":"STORAGE_ID","expiresIn":300}
curl -X PUT "$PRESIGNED_URL" --data-binary @photo.jpg -H "Content-Type: image/jpeg"
# → 200 OK
curl -X POST /api/v1/media/$STORAGE_ID/confirm \
  -H "X-API-Key: $IMAGE_API_KEY"
# → 202 {"storageObjectId":"STORAGE_ID","status":"processing"}

# 2a. Дождаться обработки (SSE или polling)
# SSE (предпочтительно):
curl -N /api/v1/media/$STORAGE_ID/status -H "X-API-Key: $IMAGE_API_KEY"
# Polling (альтернатива):
curl /api/v1/media/$STORAGE_ID -H "X-API-Key: $IMAGE_API_KEY"
# → 200 {"storageObjectId":"...","status":"COMPLETED","url":"...","contentType":"image/webp","sizeBytes":123456,"variants":[...],"createdAt":"..."}
