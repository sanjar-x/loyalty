...
import {
  ...
  publishDraft,
  rollbackVersion,
} from '@/features/pricing/api/formulas';
...
import { astToBindings, bindingsToAst } from '@/features/pricing/lib/astUtils';
...
export function FormulaTab() {
  ...
  async function handleDiscardDraft() {
    if (
      !confirm('Удалить черновик? Все несохранённые изменения будут потеряны.')
    )
      return;
    setSaving(true);
    setError(null);
  ...
  async function handlePublish() {
    if (
      !confirm(
        'Опубликовать текущий черновик? Это запустит массовый пересчёт цен.',
      )
    )
      return;
    setSaving(true);
  ...
  async function handleRollback(versionId) {
    if (
      !confirm(
        'Откатить к этой версии? Текущая опубликованная версия будет архивирована.',
      )
    )
      return;
    setSaving(true);
  ...
  if (error && versions.length === 0 && !draft)
            ...
            <>
              <div className="flex items-center justify-between">
                <h3 className="text-app-text text-sm font-semibold">
                  Редактор формулы{' '}
                  {draft
                    ? `(черновик v${draft.versionNumber})`
                    : '(новый черновик)'}
                </h3>
              ...
              <div className="border-app-border rounded-xl border bg-white p-3">
                <FormulaEditor
              ...
              <div className="flex items-center gap-2">
                ...
                {draft && (
                  <button
                    ...
                    title={
                      hasUnsavedChanges ? 'Сначала сохраните черновик' : ''
                    }
                  >
                    Опубликовать
                  </button>
