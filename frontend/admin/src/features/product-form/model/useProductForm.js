...
function mapProductToFormState(product, mediaAssets = []) {
  ...
  const variants = (product.variants ?? []).map((variant) => {
    ...
    const uniformPrice =
      ...
      firstSkuPrice;
    const uniformCompare = skus[0]?.compareAtPrice?.amount ?? null;
    ...
    return {
      ...
      priceCurrency:
        variant.defaultPrice?.currency ?? product.priceCurrency ?? 'RUB',
      variablePricing,
      perSkuPrices,
      skus: skus.map((s) => ({
        id: s.id,
    ...
    () =>
      state.variants.map((v) => {
        ...
        const skuGeneratePayload = {
          attributeSelections,
          priceAmount:
            useFlat && v.priceAmount !== '' ? toKopecks(v.priceAmount) : null,
