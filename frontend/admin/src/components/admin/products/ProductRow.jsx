...
function RowActions({ productId, status, open, onToggle, onStatusChange, onRequestArchive, onRequestDelete, onClose }) {
    ...
    return (
      <div className={cn(styles.thumb, styles.thumbPlaceholder)}>
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <path d="M3 16l5-5 4 4 3-3 6 6" stroke="#ccc" strokeWidth="1.5" strokeLinejoin="round" />
        </svg>
      </div>
    );
  }
    ...
    <div className={styles.row}>
      <div className={styles.productCell}>
        ...
        <div className={styles.productText}>
          ...
          {p.brandName && (
            <p className={styles.productBrand}>{p.brandName}</p>
          )}
          {p.createdAt && (
            ...
            <div className={styles.variantAttrsWrap}>
              {p.variantAttrs.map((group) => (
                <div key={group.name} className={styles.variantAttrGroup}>
                  ...
                  {group.values.map((v) => (
                    <span key={v} className={styles.sizeBadge}>{v}</span>
                  ))}
                </div>
              ))}
            </div>
