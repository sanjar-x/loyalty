...
import { previewPrice } from '@/services/pricing/formulas';
...
export function SandboxPanel() {
  ...
  return (
    <div className="flex flex-col gap-4 rounded-xl border border-app-border bg-[#fafafa] p-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-app-text">Sandbox</h3>
        {elapsed != null && (
          <span className="text-[11px] text-app-muted">{elapsed} мс</span>
        )}
      </div>
      ...
      {loading && (
        <div className="flex items-center gap-2 text-xs text-app-muted">
          <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-app-muted border-t-transparent" />
          Расчёт…
        </div>
      )}
        ...
        <div className="flex flex-col gap-3">
          <div className="rounded-xl bg-white p-3 shadow-sm">
            <div className="flex items-baseline justify-between">
              <span className="text-xs font-medium text-app-muted">Итого</span>
          ...
          {taggedComponents.length > 0 && (
            <div className="flex flex-col gap-1">
              <span className="text-[11px] font-medium text-app-muted">Декомпозиция</span>
              <div className="flex flex-col gap-0.5">
                {taggedComponents.map(({ key, label, value }) => {
                  ...
                  return (
                    <div key={key} className="flex items-center justify-between rounded-lg bg-white px-3 py-1.5">
                      <span className="text-xs text-app-text">{label}</span>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-medium text-app-text">
                          {formatCurrency(value)}
                        </span>
                        <span className="w-10 text-right text-[10px] text-app-muted">
                          {pct}%
                        </span>
                      </div>
                    </div>
          ...
          {result.finalPrice && components.margin && (
            <div className="rounded-lg bg-emerald-50 px-3 py-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-emerald-700">Маржа</span>
      ...
      {canPreview && (
        <button
          onClick={runPreview}
          disabled={loading}
          className="rounded-lg bg-app-text px-3 py-2 text-xs font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50"
        >
          Пересчитать
        </button>
      )}
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <input
        ...
        className="h-8 rounded-md border border-app-border bg-white px-2 font-mono text-xs text-app-text outline-none transition-colors focus:border-app-text"
      />
    </div>
  );
}
