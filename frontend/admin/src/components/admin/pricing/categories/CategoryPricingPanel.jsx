...
export function CategoryPricingPanel({
  ...
  return (
    <div className="flex flex-col gap-5 rounded-xl border border-app-border bg-white p-4">
      ...
      {categoryVariables.length > 0 && (
        <section className="flex flex-col gap-3">
          <h4 className="text-xs font-medium text-app-muted">Значения переменных (scope: category)</h4>
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h4 className="text-xs font-medium text-app-muted">Диапазоны (scope: range)</h4>
        ...
        {explicitNoRanges && rangeVariables.length > 0 && (
          <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
            Range-переменные будут использовать значения из category-scope или default.
          </div>
        )}
      </section>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {saving ? 'Сохранение…' : hasSettings ? 'Сохранить' : 'Создать настройки'}
        </button>
      </div>
    </div>
  );
