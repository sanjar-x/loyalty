...
import cn from "clsx";
// Split (сплит) to'lovi vaqtinchalik o'chirilgan — backend hali payment
// modulini qo'llab-quvvatlamaydi (Spec §11). Tayyor bo'lganda bu importni
// va quyidagi `useSplit`/`SplitPaymentSheet` bloklarini qayta yoqing.
// import SplitPaymentSheet from "@/components/blocks/product/SplitPaymentSheet";

// Recipient/customs/promo/payment endi `useCheckoutStore`'da (sessionStorage
// persist). Card detail saqlanmaydi — PCI DSS spirit'i: brauzer hech qachon
// PAN/CVC/expiry/holder ga to'liq egalik qilmaydi. To'lov vaqti payment
// provider widget (e.g. Tinkoff PayLink) ochiladi.
...
function CheckoutPageInner() {
  ...
  // Backend orchestrator. Cart→checkout transition `useCheckoutFlow` orqali
  // boshqariladi: selection (sessionStorage), quote, initiate→confirm.
  ...
  // URL search params'dan flow store'ga sync qilish — `/checkout/pickup`
  // sahifasi tanlovni `?pickupPvzId=...` orqali qaytaradi. Buni yagona
  // canonical store'ga ko'chiramiz, shunda quote va checkout API'lar
  // doim store'dan o'qiydi (URL emas).
  //
  // ⚠ providerCode majburiy — `useGetRateQuoteMutation` snake_case body'da
  // `provider_code` jo'natadi va u backend `LogisticsProviderCodeEnum` validation'iga
  // tushadi. Default fallback (eski "|| 'cdek'") yashirin xato keltiradi:
  // foydalanuvchi Yandex PVZ tanlasa, biz CDEK'ga rate quote chaqiramiz va
  // 422 olamiz. Endi providerCode yo'q bo'lsa pickup sync skip qilinadi —
  // `/checkout/pickup` doim o'rnatadi (`params.set("pickupProvider", ...)`).
  useEffect(() => {
  ...
  }, [pickup.pickupPvzId, pickup.pickupProvider, pickup.pickupAddress, flow]);

  // Buyurtma yakunlangach navigate
  useEffect(() => {
    if (flow.status === CheckoutStatus.CONFIRMED) {
      flow.onConfirmed();
    }
  }, [flow.status, flow]);

  // Backend xatolarini toast bilan surfacing — `flow.error` o'zgarganda
  // (QUOTE_EXPIRED auto-refresh, ATTEMPT_EXPIRED background watcher,
  // refreshQuote provider unavailable, va h.k.). Pay button ichidagi
  // alohida toast.error ham bor — duplikat bo'lmasligi uchun bu effect
  // faqat `flow.error.code != null` (auto-fire'lar) ni ko'rsatadi.
  const lastErrorIdRef = React.useRef(null);
  useEffect(() => {
  ...
  // Sahifadan chiqishda frozen cart'ni unfreeze qilish (best-effort).
  //
  // Eski versiyada `flow.status`/`flow.abandon` initial render closure'idan
  // o'qilardi (eslint-disable bilan), shuning uchun cleanup amalda doim
  // IDLE bilan ishlardi va hech qachon abandon bajarmasdi. To'g'ri yondashuv:
  //  1. Joriy status'ni `useCheckoutStore.getState()` orqali olish (snapshot).
  //  2. `flow.abandon`'ni har render'da yangilanadigan ref'ga yozish — cleanup
  //     LATEST callback'ni chaqiradi (bu ham backend cancel mutation'ini, ham
  //     local store reset'ni bajaradi).
  const abandonRef = React.useRef(flow.abandon);
  useEffect(() => {
  ...
  // Recipient/customs/promo/payment state — Zustand store'da persist
  // (sessionStorage). LocalStorage hidratsiyasi va format-tekshiruvi keraksiz —
  // Zustand persist middleware'i o'zi qiladi.
  ...
  // Card draft faqat sheet ochiq turganda mavjud. Submit'da hech qaerga
  // saqlanmaydi — sheet yopilgach state tashlab yuboriladi. PCI: brauzer
  // hech qachon PAN/CVC/expiry/holder'ga to'liq egalik qilmaydi; haqiqiy
  // to'lov vaqti payment provider widget (Tinkoff PayLink va h.k.) ochiladi
  // va token muomalada bo'ladi.
  ...
  const closeCardModal = () => {
    ...
    setCardDraft({ numberDigits: "", exp: "", cvc: "", holder: "" });
  ...
  // (Spec §11). State'ni saqlab turamiz (setter chaqiriqlari ishlashi
  // uchun), lekin UI toggle va SplitPaymentSheet kommentga olingan, shuning
  // uchun u doim `false` bo'lib qoladi va `payButtonTitle` non-split
  // branch'ga tushadi. Tayyor bo'lganda kommentlarni olib tashlang.
  ...
  // Selected items — UI-mapped shape kerak (`name, image, size, article,
  // priceRub, deliveryText, shippingText`). `useCart()` shu shape'ni
  // beradi (`useCart.js#items`). `flow.selectedItems` raw `CartItemResponse`
  // qaytaradi (`productName, imageUrl, variantLabel`) — UI uchun mos emas.
  //
  // Strategiya: canonical selection = `flow.selectedSkuIds` (Zustand store);
  // UI rendering = `useCart().items.filter(skuId in selection)`. Ikkala
  // manba ham bitta backend so'rovidan keladi (RTKQ cache hit).
  //
  // selectedSkuIds bo'sh → butun cart (foydalanuvchi `/checkout`'ga
  // to'g'ridan-to'g'ri kirgan, default sifatida hammasini olish UX'ga moslashadi).
  ...
  // Yagona pure totals hisoblovchi (`lib/checkout/totals.js`).
  // - `flow.quote` haqiqiy delivery narxini beradi (kopecks → ruble).
  // - Loyalty Points moduli backend'da yo'q — `points` props'i hozir doim null,
  //   shu sabab `pointsRub: 0`. Modul tayyor bo'lganda `useMyPointsQuery` orqali
  //   `{ applied: <value> }` ulanadi.
  ...
  const {
    ...
    subtotalRub: selectedSubtotalRub,
    deliveryRub,
    discountRub,
    pointsRub,
    totalRub,
  ...
  // Card last4 endi store'da saqlanmaydi — payment provider widget'i
  // muvaffaqiyatli token qaytarganda backend'dan kelgan masked PAN
  // ko'rsatilishi kerak. Hozircha pay button hech qanday suffix bermaydi.
        ...
        <div className={styles.c13}>
          ...
          <div className={styles.c53}>
            <div className={cn(styles.c54, styles.tw18)}>
              ...
              {/* Saqlangan kartalar — backend payment moduli yoqilganda
                  bu yerga foydalanuvchining masked PAN'lari (last4 + brand)
                  keladi. Hozircha "Добавить карту" tugmasi har safar yangi
                  card detail kiritishni so'raydi (yoki kelajakda payment
                  provider widget'iga yo'naltiradi). */}
              <button
                type="button"
            ...
            <div className={cn(styles.c60, styles.spaceY2)}>
              ...
              {/* Списать баллы — backend Loyalty Points moduli (Spec §11)
                  yoqilgandan keyin `usePoints` `pointsRub`'ni `totals`'da
                  hisoblanishiga ulanadi. Hozircha toggle visual'gina ishlaydi,
                  chegirma 0 ₽ ko'rsatadi. */}
              <div className={styles.c73}>
                ...
                <div className={cn(styles.c75, styles.tw24)}>
                  <span className={styles.c76}>
                    {pointsRub > 0 ? `-${formatRub(pointsRub)}` : ""}
                  </span>
                  <button
                    type="button"
          ...
          <div>
            <div className={styles.c95}>
              <button
                ...
                disabled={
                  selectedQuantity === 0 ||
                  flow.status === CheckoutStatus.INITIATING ||
                  flow.status === CheckoutStatus.CONFIRMING ||
                  !flow.canPlaceOrder
                }
                onClick={async () => {
                  if (selectedQuantity === 0) return;
                  // Split disabled — backend payment moduli yo'q (Spec §11).
                  // if (useSplit) {
                  //   setIsSplitSheetOpen(true);
                  //   return;
                  // }
                  // Spec §7.4: hozirgi backend faqat `pickupPointId` qabul
                  // qiladi. Recipient/customs/promo/payment integratsiyasi
                  // hali backend'da yo'q. `placeOrder` initiate→confirm
                  // pipelinini bajaradi va `orderId` qaytaradi (yoki null).
                  // placeOrder ichidagi xatolar `setError` orqali store'ga yoziladi
                  // va yuqoridagi `flow.error` watcher effect avtomatik toast.error
                  // chaqiradi — bu yerda alohida toast yo'q (duplikat bo'lmasin).
                  await flow.placeOrder();
                  // Muvaffaqiyatli bo'lsa CONFIRMED → useEffect
                  // `flow.onConfirmed()` chaqirib navigatsiya qiladi.
                }}
                className={`${cn(
                  styles.payButton,
                  selectedQuantity > 0
              ...
              {/* Split sheet vaqtinchalik o'chirilgan (yuqoridagi sabab) */}
              {/*
              <SplitPaymentSheet
              ...
              <div className={styles.c159}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    // PCI: card detail brauzerda saqlanmaydi. Faqat
                    // `paymentMethod = "card"`'ni store'ga yozamiz —
                    // haqiqiy to'lov vaqti payment provider widget'i
                    // (Tinkoff PayLink, ЮKassa) ochiladi va token muomalada
                    // bo'ladi. Sheet yopilganda cardDraft tashlab yuboriladi.
                    setPaymentMethod("card");
                    setCardDraft({
                      numberDigits: "",
                      exp: "",
