...
interface ReturnRequest {
  ...
  title: string;
  statusText: string;
  statusType: "approved" | "review" | "rejected" | "closed";
  disabled?: boolean;
...
function saveReturnDraft(payload: Record<string, unknown>): void {
  try {
...
function StatusPill({ type, children }: StatusPillProps) {
  const className = (() => {
  ...
  return (
    ...
    >
      <div className={styles.cardTop}>
        <div className={styles.cardNo}>&numero;{req.no}</div>
      ...
      <div className={styles.productRow}>
        ...
        <div className={styles.productMeta}>
          ...
          <div className={styles.productSub}>
            Размер: <span>{req.product.size}</span> &nbsp;&middot; Артикул:{" "}
...
function FaqRow({ icon, title, text }: FaqRowProps) {
  return (
...
function ReturnOrderCard({ order }: ReturnOrderCardProps) {
  const router = useRouter();
            ...
            <div className={styles.returnProductRow}>
              ...
              <div className={styles.returnProductMeta}>
                ...
                <div className={styles.returnProductSub}>
                  Размер: <span>{it.size}</span> &middot; Артикул:{" "}
...
export default function ReturnsPage() {
  const [tab, setTab] = useState<"requests" | "return">("requests");
  ...
  const returnOrders = useMemo<ReturnOrder[]>(
    () => [
      {
