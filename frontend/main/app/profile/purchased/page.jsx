...
export default function PurchasedPage() {
  ...
  // Backend Orders/Purchased moduli yo'q (Spec §11) — ro'yxat bo'sh boshlanadi.
  // Modul tayyor bo'lganda RTKQ endpoint'ga (masalan, `useListOrdersQuery` yoki
  // dedicated `usePurchasedItemsQuery`) ulanadi va shape o'sha format'ga moslashadi.
  ...
  // "Для вас" — for-you feed; backend kelgan content. Sotib olingan tovarlar
  // ro'yxatidan farqli, bu real RTKQ endpoint orqali keladi.
  const { data: forYouResponse } = useGetForYouFeedQuery({ limit: 8 });
  const forYouProducts = useMemo(() => {
    const items = Array.isArray(forYouResponse?.items)
      ? forYouResponse.items
      : [];
