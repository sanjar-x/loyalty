...
export default function CategoryPage(): React.JSX.Element {
  ...
  const categoryParam = safeDecode(categoryParamRaw as string | undefined);

  const categoriesWithTypes: CategoryWithTypes[] = [];
  const isLoading = false;
  const isError = false;

  const legacyNameByKey: Record<string, string> = {
    clothes: "Одежда",
    shoes: "Обувь",
  ...
  const types: CategoryType[] = Array.isArray(category?.types)
    ? category.types
    : [];
