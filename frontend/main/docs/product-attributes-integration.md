...
const { id: productId, defaultVariantId } = await api.post('/catalog/products', {
...
await api.patch(`/catalog/products/${productId}/status`, { status: 'enriching' });
// ... после review ...
await api.patch(`/catalog/products/${productId}/status`, { status: 'published' });
```
