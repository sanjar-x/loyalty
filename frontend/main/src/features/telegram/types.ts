...
export interface DeviceStorage {
   ...
   * @param callback - Callback with error and the stored value.
   */
  getItem(key: string, callback: (error: string | null, value: string) => void): DeviceStorage;
...
export interface WebApp {
   ...
   * @since Bot API 6.9
   */
  requestWriteAccess(callback?: (status: WriteAccessStatus) => void): void;
   ...
   * @since Bot API 6.9
   */
  requestContact(callback?: (status: ContactRequestStatus) => void): void;
