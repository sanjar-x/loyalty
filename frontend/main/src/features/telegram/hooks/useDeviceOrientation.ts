...
export function useDeviceOrientation(refreshRate: number = 1000, needAbsolute: boolean = false) {
  const isAvailable = supportsFeature('DeviceOrientation');
  ...
  const start = useCallback(() => {
    ...
    safeCall(() => {
      wa.DeviceOrientation.start({ refresh_rate: rate, need_absolute: needAbsolute }, (started) => {
        if (started) {
