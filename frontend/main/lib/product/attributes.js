...
export function getSizeGuideUrl(mediaAssets) {
  ...
  const asset = mediaAssets.find(
    (m) => m?.role === "size_guide" && m.url?.trim(),
  );
...
// Backend SKU `variantAttributes` ichidan razmer atributini topish.
// Substring matching: backend turli domain'larda `size`, `clothing_size`,
// `shoe_size`, va ruschasiga `Размер`, `Размер одежды`, `Размер обуви` kabi
// nomlar qaytaradi — barchasini bitta tekshiruv qoplaydi.
...
// Razmer label uchun short alpha code'ni normalize qilish: `"m"` → `"M"`.
// Numeric (`"42"`) va uzun string (`"one size"`) lar tegmaydi.
...
// Atribut juftligidan ko'rinadigan label tanlash:
// `value` (denormalized display) → `valueI18N.ru` → `valueI18N.en`
// → `valueCode` (uppercase qilingan).
...
export function deriveSizesFromSkus(skus) {
  ...
  for (const sku of skus) {
    ...
    result.push({
      ...
      sortOrder:
        typeof sizeAttr?.sortOrder === "number" ? sizeAttr.sortOrder : null,
    });
  }

  // sortOrder mavjud bo'lsa shunga ko'ra (XS<S<M<L<XL); aks holda asl tartib.
