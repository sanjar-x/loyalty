...
import { mapStorefrontProduct } from "@/lib/format/mapStorefrontProduct";
...
export const loadProductBySlug = cache(async (slug) => {
  ...
  const url = `${base}/api/v1/catalog/storefront/products/${encodeURIComponent(
    slug,
  )}`;
  ...
  if (!res.ok) {
    ...
    if (res.status >= 500) {
      const err = new Error(
        `Upstream ${res.status} while fetching product "${slug}"`,
      );
