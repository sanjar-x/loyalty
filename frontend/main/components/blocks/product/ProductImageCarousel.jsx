 ...
 * - Touchdagi swipe va mouse drag ikkalasi ham ishlaydi (browser native).
 * - Tap vs swipe ajratish: pointerdown/up delta > 8px bo'lsa `onTap` chaqirilmaydi
 *   (card click'ni bosmaslik uchun).
 * - Fallback: 1 rasm bo'lsa — static <Image/> renderlanadi, scroll yo'q.
 */
function ProductImageCarousel({
  ...
  const onClickCapture = useCallback((e) => {
    // Mouse drag tugagandan keyin chiqadigan click'ni yutib olamiz
    if (didSwipe.current) {
      e.preventDefault();
      e.stopPropagation();
      didSwipe.current = false;
  ...
  // Arrow-key navigation uchun accessibility
  ...
  return (
    ...
    >
      ...
      >
        {list.map((src, i) => {
          // Faqat yaqin slaydlarni eager yuklaymiz
