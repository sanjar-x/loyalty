"""Generic Data Mapper repository — shared kernel (REC-031).

Promoted out of ``catalog/infrastructure/repositories/base.py``. The
catalog-specific ``ICatalogRepository[EntityType]`` parent has been
dropped here; the canonical generic CRUD contract is
:class:`shared.interfaces.repositories.IBaseRepository`.

Module-specific subclasses opt in by:

* Inheriting from this class with their domain entity / ORM model
  type parameters.
* Supplying ``model_class=...`` in the subclass declaration so the
  ``__init_subclass__`` hook stamps the class-level ``model``
  attribute used by generic CRUD.
* Implementing the abstract ``_to_domain`` / ``_to_orm`` mapping
  hooks. The Data-Mapper rule: domain entities never reference ORM
  rows, ORM rows never reference domain entities — both directions go
...
from shared.interfaces.entities import IBase
from shared.interfaces.repositories import IBaseRepository


class BaseRepository[EntityType, ModelType: IBase](IBaseRepository[EntityType]):
    """Generic Data Mapper repository.

    Accepts and returns only domain entities (``EntityType``).
    Subclasses declare the ORM model via the ``model_class`` class
    argument and implement the ``_to_domain`` / ``_to_orm`` mapping
    hooks.
    ...
    async def update(self, entity: EntityType) -> EntityType:
        ...
        re-raises via :meth:`_translate_integrity_error` so that
        subclasses can map them to domain exceptions.
    ...
    def _translate_integrity_error(
        ...
        Subclasses override this to inspect the constraint name and
        raise the appropriate domain-specific error. The default
        implementation is a no-op, which lets the original
